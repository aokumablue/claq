...
- 記憶の単位は **知識カード**（`knowledge` の 1 行）。`kind` は `convention` / `decision` / `pitfall` / `howto` / `fact` / `preference` の 6 値、`scope` は `global` / `repo`。SessionStart に注入されるのは `status='active'` のみで、`pending` は人間が `/instinct promote` で昇格させるまで注入されない。`status` の既定値は `source` に依存する（A-03）: `source=human` は `active`、`source=agent`/`observer`（`bluecore_mem_learn` 経由のカードは常にこれ）は `pending` — つまり agent 由来カードは `--status active` を明示しない限り `/instinct promote` を経ないと注入されない
