...
_WRAPPER = '"${CLAUDE_PLUGIN_ROOT}/runtime/claq-hook"'
...
class TestSecurityPreflightHook:
    ...
    def test_real_guard_module_passes(self, tmp_path: Path) -> None:
        ...
        _write_hooks_json(
            ...
            {
                "PreToolUse": [
                    {"hooks": [{"type": "command", "command": f"{_WRAPPER} claq.hooks.block_no_verify"}]}
                ]
            },
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                {
                    "PreToolUse": [
                        {"hooks": [{"type": "command", "command": f"{_WRAPPER} claq.hooks.pre_compact"}]}
                    ]
                },
            ),
            ("空配列", {"PreToolUse": []}),
            (
                ...
                {
                    "SessionStart": [
                        {"hooks": [{"type": "command", "command": f"{_WRAPPER} claq.hooks.config_protection"}]}
                    ]
                },
            ),
        ],
