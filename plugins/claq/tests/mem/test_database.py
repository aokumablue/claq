"""claq.mem.database — repos / sessions / knowledge の 3 テーブルのテスト。"""
...
from claq.mem.database import Database, DatabaseError
...
def _repo(repo_id: str = "claq-dev", identity_key: str = "git@github.com:x/claq.git") -> Repo:
    """テスト用 Repo を組み立てる。"""
    return Repo(
        id=repo_id,
        identity_key=identity_key,
        root_path="/Users/x/dev/claq-dev",
        remote_url="git@github.com:x/claq.git",
        first_seen_at=_TS,
        last_seen_at=_TS,
    )
...
class TestSchema:
    ...
    def test_global_scope_rejects_repo_id(self, db: Database) -> None:
        ...
        with pytest.raises(sqlite3.IntegrityError):
            db.upsert_knowledge(_knowledge(scope="global", repo_id="claq-dev"))
    ...
    def test_repo_delete_cascades_to_knowledge(self, db: Database) -> None:
        ...
        db.upsert_repo(_repo())
        db.upsert_knowledge(_knowledge(scope="repo", repo_id="claq-dev"))
        db.conn.execute("DELETE FROM repos WHERE id = ?", ("claq-dev",))
        db.conn.commit()
...
class TestConnection:
    ...
    def test_parent_dir_under_claq_is_0700(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """~/.claq 配下に作る DB の親ディレクトリは umask 022 でも 0700。"""
        import os
        ...
        try:
            db_path = tmp_path / ".claq" / "mem.db"
            with Database(db_path):
                pass
            assert stat.S_IMODE((tmp_path / ".claq").stat().st_mode) == 0o700
        finally:
            os.umask(old_umask)
...
class TestRepos:
    ...
    def test_upsert_inserts_new_repo(self, db: Database) -> None:
        ...
        assert stored.id == "claq-dev"
        assert stored.identity_key == "git@github.com:x/claq.git"
    ...
    def test_upsert_resolves_conflict_by_identity_key(self, db: Database) -> None:
        ...
        moved = Repo(
            id="別のスラッグ",
            identity_key="git@github.com:x/claq.git",
...
class TestKnowledge:
    ...
    def test_same_key_conflicts_within_same_repo(self, db: Database) -> None:
        ...
        db.upsert_repo(_repo())
        db.upsert_knowledge(_knowledge(scope="repo", repo_id="claq-dev"))
        db.upsert_knowledge(_knowledge(scope="repo", repo_id="claq-dev", title="上書き"))
    ...
    def test_upsert_persists_all_optional_columns(self, db: Database) -> None:
        """任意カラムがすべて往復する。"""
        db.upsert_repo(_repo())
        session = db.start_session(Session(session_uid="uid-1", repo_id="claq-dev"))
        ...
        stored = db.upsert_knowledge(
            _knowledge(
                ...
                status="pending",
                source="observer",
                source_ref="plugins/claq/src/claq/mem/database.py",
    ...
    def test_get_by_key_scoped_to_repo(self, db: Database) -> None:
        ...
        db.upsert_repo(_repo())
        db.upsert_knowledge(_knowledge(scope="repo", repo_id="claq-dev", title="repo 側"))
        db.upsert_knowledge(_knowledge(title="global 側"))
        found = db.get_knowledge_by_key("use-python3", repo_id="claq-dev")
        assert found is not None
    ...
    def test_list_with_all_filters(self, db: Database) -> None:
        ...
        db.upsert_knowledge(_knowledge("hit", scope="repo", repo_id="claq-dev"))
        db.upsert_knowledge(_knowledge("wrong-repo", scope="repo", repo_id="other"))
        db.upsert_knowledge(_knowledge("wrong-scope"))
        db.upsert_knowledge(
            _knowledge("wrong-status", scope="repo", repo_id="claq-dev", status="archived")
        )
        rows = db.list_knowledge(scope="repo", repo_id="claq-dev", status="active")
...
class TestSessions:
    ...
    def test_start_session_inserts(self, db: Database) -> None:
        ...
        stored = db.start_session(
            Session(session_uid="uid-1", repo_id="claq-dev", harness="claude", started_at=_TS)
        )
        assert stored.id is not None
    ...
    def test_start_session_is_idempotent(self, db: Database) -> None:
        ...
        first = db.start_session(
            Session(session_uid="uid-1", repo_id="claq-dev", started_at=_TS)
        )
    ...
    def test_finish_session_records_handoff(self, db: Database) -> None:
        ...
        db.upsert_repo(_repo())
        db.start_session(Session(session_uid="uid-1", repo_id="claq-dev"))
        assert db.finish_session("uid-1", "次はテストを書く", ended_at="2026-01-02T00:00:00+00:00") is True
        latest = db.get_latest_session("claq-dev")
    ...
    def test_get_latest_session_skips_empty_handoff(self, db: Database) -> None:
        ...
        assert db.get_latest_session("claq-dev") is None
    ...
    def test_get_latest_session_returns_newest_with_handoff(self, db: Database) -> None:
        ...
        for uid, started in (("uid-1", _TS), ("uid-2", "2026-02-01T00:00:00+00:00")):
            db.start_session(Session(session_uid=uid, repo_id="claq-dev", started_at=started))
