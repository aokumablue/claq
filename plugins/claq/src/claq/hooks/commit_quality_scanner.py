...
from claq.lib.core_utils import log
...
_SECRET_PATTERNS: tuple[tuple[str, str], ...] = (
    # ベンダ prefix の集合は `claq.mem.redaction._PATTERNS` と揃える。同じ
...
class _ScanBudget:
    ...
    最初の `check()` は必ず時刻を読みます。

    **共有されるのは `deadline` だけで、バイト数の積算はインスタンス単位**です。
    本クラスはスキャナ呼び出しごと（＝ファイルごとに lint 用と secret 用の
    2 個）生成されるため、積算はファイルを跨いで持ち越されません。したがって
    `_SCAN_BUDGET_CHECK_BYTES` が実際に発火するのは「1 ファイルの中で
    256KiB 以上を走査したとき」だけで、256KiB 未満の小さなファイルが N 件
    並ぶケースでは 1 ファイルにつき初回の 1 回しか時刻を読みません。

    それで足りるのは、実時間の上界を与えているのが積算間隔ではなく共有
    `deadline` の方だからです。初期残量 0 によって**各ファイル・各スキャナが
    必ず開始時に 1 回は deadline を見る**ので、予算が尽きていれば次のファイル
    （あるいは同じファイルの secret scan）が必ず fail-closed で止まります。
    積算間隔が受け持つのは「1 ファイルの中を走り抜ける間の取りこぼし」——
    まさに旧「行数」基準が改行の無い巨大 1 行で落としていた区間——だけです。
    """
...
def _sanitize_untrusted_excerpt(text: str) -> str:
    ...
    （``</claq-memory>`` など）を `strip_tags` で倒し、長さにも上限を置く。
