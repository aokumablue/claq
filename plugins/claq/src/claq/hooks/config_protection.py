...
# パス連続一致なら ``plugins/claq/hooks/hooks.json`` とプラグインキャッシュ配下
...
CONDITIONALLY_PROTECTED_FILES = frozenset({
    ...
    "tox.ini",
    "package.json",
    # ホスト設定。保護フックを止める手段を 3 つ持つ（`env` の `CLAQ_PYTHON`、
    # `hooks` の差し替え、プラグインの無効化）。とくに `CLAQ_PYTHON` は
    # `runtime/claq-hook` が exec するため、保護の無効化ではなく**全ツール
    # 呼び出しでの任意コード実行**になる。
    #
...
# `env` 全般ではなく `CLAQ_PYTHON` の出現そのものを見る（上の理由）。
# `hooks` は PreToolUse ガードの差し替え、`enabledPlugins` / `disabledPlugins` は
# プラグインごとの無効化に対応する。
_SETTINGS_GUARD_KEYS = ("CLAQ_PYTHON", '"hooks"', '"enabledPlugins"', '"disabledPlugins"')
...
def _text_has_protected_signal(text: str, file_name: str) -> bool:
    ...
    キー（`CLAQ_PYTHON` / `hooks` / `enabledPlugins` / `disabledPlugins`）を、
...
def _conditional_blocked_message(file_name: str) -> str:
    ...
    return (
        ...
        "protection hooks: CLAQ_PYTHON, hooks, enabledPlugins/disabledPlugins). "
