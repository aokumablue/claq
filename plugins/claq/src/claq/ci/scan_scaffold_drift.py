...
引き継ぎがノイズで埋まる（docs/adr/untrusted-input-prompt-boundary.md の残存リスク筆頭）。実際に本リポジトリでは
...
見たいのはまさにその種のメッセージであり、ここで見落とすと docs/adr/staleness-detection.md の
「素通り」がそのまま残る。
...
内側には現れない。フェンスが閉じていない場合はテキストを一切触らない（docs/adr/untrusted-input-prompt-boundary.md で
閉じた「終端の来ない入力を EOF まで飲み込む」型を隣のモジュールで再現しないため）。

CI へは組み込まない。コーパスは各利用者のローカルにしか無く、コミットできない
（未サニタイズかつユーザー固有）。人間が ``maintain`` / ``release-verify`` から
明示的に呼び出す診断として置く（呼び出し手順はそれぞれの SKILL.md が持つ。
本ファイルへ複製しない）。
終了コードは ``0`` = ドリフトなし / ``1`` = ドリフト検出 / ``2`` = 走査対象ゼロ。
``2`` を ``0`` と区別するのは、走査できなかったことを「異常なし」と読ませないため
（docs/adr/verification-scope-release-gates.md の「skip されるゲートはゲートとして機能しない」と同じ理由）。
"""
