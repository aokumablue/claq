...
- 到達可能性を実行で確かめられない指摘 → **severity は「真だった場合の影響」で付け、
  `未計測` を別の軸として併記する。確信度で severity を下げない。** 加えて
  **それを計測に変える手段を 1 行添える**（叩くコマンド、または何を観測すれば決着するか）
  ...
  実測での必要性（2026-09-07）: hooks 監査で「O(N²)・MEDIUM・未確認」と返した指摘を
  親が測り直したところ、**stdin 上限の 9.2%（96KB）の入力で timeout 15 秒を超過**して
  いた。host が kill した hook は exit code を返さないため保護の完全なバイパスで、
  実体は HIGH だった。severity を確信度で丸めたせいで、真の HIGH が MEDIUM の山に
  埋もれていた

**スコープと確信度は別の軸である。** スコープ外は「この変更の欠陥ではない」ので severity を
下げてよい（下の実測が根拠）。確信度は「影響の大きさ」と無関係なので、確信度を理由に severity を下げてはならない（上の実測が根拠）。
二つの実測は逆を向いているように見えるが、別の軸を測っている。

実測での必要性（2026-08-31 時点）: 差分を取得できないまま監査した回で、`Blockers: 8` のうち 4 件が偽陽性だった。4 件はいずれも差分が触れていない既存の fail-open 分岐で、しかも全件が CRITICAL/HIGH へ入っていた。読み手が到達可能性を実行で確かめられない以上、あらゆる fail-open 分岐は生きた脆弱性に見える — これは不注意ではなく権限から構造的に導かれるため、規定で塞ぐ。
