...
**迷ったら `scope: repo`** — global を汚染するより、後で `claq_run claq.mem.cli promote <key>` できる repo 側に置くほうが安全。
...
`claq_mem_learn`（および `mem.cli learn` そのもの）は `--source` / `--status` を
...
削除ではなく `claq_run claq.mem.cli forget <key>` で `archived` にする。
置き換えた場合は `claq_run claq.mem.cli forget <old-key> --superseded-by <new-key>` で置換関係を残す。
...
`<claq-memory>` 注入で起動（SessionStart の `mem context`。`status='active'` のみ）。

search: `claq_run claq.mem.cli search "..."`（`. "$HOME/.claq/env.sh"` 前提）— クエリ例 `knowledge {domain}` / `{key}`。返るのは title 1 行だけ。本文が要る key だけ `claq_run claq.mem.cli show <key>`
record: 上記「記録する / しない」に従い、再利用可能な学びだけ `claq_mem_learn` で登録する
