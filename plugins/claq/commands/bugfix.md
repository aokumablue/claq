...
- 参照: `claq_run claq.mem.cli search "..."`（`. "$HOME/.claq/env.sh"` 前提。クエリ例 `bug fix regression repro root cause verify` / `{対象ファイルパス}` / `{症状キーワード}`）→ 本文が要る key だけ `claq_run claq.mem.cli show <key>`
