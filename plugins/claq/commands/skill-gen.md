...
- 参照: `claq_run claq.mem.cli search "..."`（`. "$HOME/.claq/env.sh"` 前提。クエリ例 `skill-gen pattern repository workflow`）→ 本文が要る key だけ `claq_run claq.mem.cli show <key>`
...
1. **`claq:grader`**: 実行トランスクリプトと出力を期待値と照合し合否と根拠を整理
2. **`claq:comparator`**: 改善前後（または2候補）の出力をブラインド比較しどちらが課題達成度が高いか判定
3. **`claq:bench-analyzer`**: ベンチマーク結果と比較結果を要約し勝因・性能傾向を抽出
