...
- 参照: `claq_run claq.mem.cli search "..."`（`. "$HOME/.claq/env.sh"` 前提。クエリ例 `harness audit score` / `harness config optimization audit` (days: 90)）→ 本文が要る key だけ `claq_run claq.mem.cli show <key>`
...
スコアカードを出力。`--audit-only` は /harness レベルの制御フラグであり `claq_run` へ渡さない。指定時はここで終了。`--root` / `--target-kind` はステップ4のベースライン比較まで同一値を保持する。
...
`claq:harness-tuner` を起動。ベースラインJSONとトップ3アクションを渡し、信頼性・コスト・スループット最適化を委譲。
...
- `--audit-only`: ステップ1のみで終了（/harness レベルの制御フラグ。`claq_run` へ渡さない）
