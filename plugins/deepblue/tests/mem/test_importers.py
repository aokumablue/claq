...
from deepblue.mem.importers import (
...
class TestImportInstincts:
    ...
    def test_no_instincts_dir_returns_zero(self):
        from deepblue.mem.importers import import_instincts

        db = MagicMock()
        # 存在しないパスをモック
        with patch("deepblue.mem.importers.DEEPBLUE_DIR", Path("/nonexistent")):
    ...
    def test_parses_yaml_instinct_files(self, tmp_path):
        ...
        instincts_dir = tmp_path / ".deepblue" / "instincts" / "personal"
...
"""
        ...
        with patch("deepblue.mem.importers.DEEPBLUE_DIR", tmp_path / ".deepblue"):
            result = import_instincts(db, "test_user")
...
""",
    ...
    def test_project_filter_skips_global_dirs(self, tmp_path):
        ...
        db = MagicMock()
        global_dir = tmp_path / ".deepblue" / "instincts" / "personal"
        project_dir = tmp_path / ".deepblue" / "projects" / "proj-1" / "instincts" / "personal"
        other_project_dir = tmp_path / ".deepblue" / "projects" / "proj-2" / "instincts" / "personal"
        global_dir.mkdir(parents=True)
        project_dir.mkdir(parents=True)
        other_project_dir.mkdir(parents=True)
        (tmp_path / ".deepblue" / "projects" / "skip.txt").write_text("ignore", encoding="utf-8")
    ...
    def test_duplicate_project_dirs_are_skipped(self, tmp_path):
        ...
        first = deepblue_dir / "projects" / "proj-1" / "instincts" / "personal"
        first.mkdir(parents=True)
        (first / "first.yaml").write_text("id: first\nconfidence: 0.6\n", encoding="utf-8")

        with patch("deepblue.mem.importers.DEEPBLUE_DIR", deepblue_dir):
            result = import_instincts(db, "test_user")
    ...
    def test_same_project_in_multiple_dirs_is_skipped(self, tmp_path):
        ...
        with patch("deepblue.mem.importers._project_dirs", lambda: [dir_a, dir_b]):
            result = import_instincts(db, "test_user")
    ...
    def test_event_logs_same_project_in_multiple_dirs_is_skipped(self, tmp_path):
        ...
        from deepblue.mem.importers import import_event_logs
...
""",
        ...
        with patch("deepblue.mem.importers.Path.read_text", fake_read_text):
        ...
        with patch("deepblue.mem.importers.Path.read_text", side_effect=OSError("boom")):
...
class TestImportEventLogs:
    ...
    def test_no_log_files_returns_zero(self, tmp_path):
        ...
        # 存在しないディレクトリをモック
        with (
            patch("deepblue.mem.importers.DEEPBLUE_DIR", tmp_path / "nonexistent" / ".deepblue"),
            patch("deepblue.mem.importers.DEEPBLUE_STATE_DIR", tmp_path / "nonexistent" / "state"),
        ):
    ...
    def test_parses_jsonl_files(self, tmp_path):
        ...
        obs_file = deepblue_dir / "observations.jsonl"
        obs_file.write_text(
        ...
        with (
            patch("deepblue.mem.importers.DEEPBLUE_DIR", deepblue_dir),
            patch("deepblue.mem.importers.DEEPBLUE_STATE_DIR", state_dir),
        ):
            result = import_event_logs(db, "test_user")
    ...
    def test_import_event_logs_covers_global_project_and_secondary_files(self, tmp_path):
        ...
        db = MagicMock()
        deepblue_root = tmp_path / ".deepblue"
        state_dir = tmp_path / ".claude" / "state"
        project_dir = deepblue_root / "projects" / "proj-1"
        project_dir.mkdir(parents=True)
        state_dir.mkdir(parents=True)
        (deepblue_root / "observations.jsonl").parent.mkdir(parents=True, exist_ok=True)
        (deepblue_root / "observations.jsonl").write_text(json.dumps({"payload": "global"}) + "\n", encoding="utf-8")
        ...
        logs_dir = deepblue_root / "logs"
        logs_dir.mkdir(parents=True)
        (logs_dir / "costs.jsonl").write_text(json.dumps({"payload": "cost"}) + "\n", encoding="utf-8")

        with (
            patch("deepblue.mem.importers.DEEPBLUE_DIR", deepblue_root),
            patch("deepblue.mem.importers.DEEPBLUE_STATE_DIR", state_dir),
        ):
            result = import_event_logs(db, "test_user")
    ...
    def test_import_event_logs_honors_project_filter(self, tmp_path):
        ...
        db = MagicMock()
        deepblue_dir = tmp_path / ".deepblue"
        project_dir = deepblue_dir / "projects" / "proj-1"
        other_dir = deepblue_dir / "projects" / "proj-2"
        project_dir.mkdir(parents=True)
    ...
    def test_duplicate_project_dirs_are_skipped(self, tmp_path):
        ...
        db = MagicMock()
        deepblue_dir = tmp_path / ".deepblue"
        first = deepblue_dir / "projects" / "proj-1"
        first.mkdir(parents=True)
...
class TestImportAll:
    ...
    def test_calls_all_importers(self, tmp_path):
        from deepblue.mem.importers import import_all
    ...
    def test_import_all_aggregates_counts(self, tmp_path):
        ...
        db = MagicMock()
        with (
            patch("deepblue.mem.importers.import_instincts", return_value=1),
            patch("deepblue.mem.importers.import_adrs", return_value=2),
            patch("deepblue.mem.importers.import_event_logs", return_value=3),
        ):
