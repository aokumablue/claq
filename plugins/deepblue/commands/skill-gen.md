...
1. **`deepblue:grader`**: 実行トランスクリプトと出力を期待値と照合し合否と根拠を整理
2. **`deepblue:comparator`**: 改善前後（または2候補）の出力をブラインド比較しどちらが課題達成度が高いか判定
3. **`deepblue:bench-analyzer`**: ベンチマーク結果と比較結果を要約し勝因・性能傾向を抽出
