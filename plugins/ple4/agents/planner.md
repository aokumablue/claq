...
## Phase 1: オプション追加
1. **引数定義追加** (plugins/ple4/src/ple4/mem/cli.py)
   - Action: `_VALUE_OPTIONS` に `--days` を追加し `search` サブコマンドで受理する
   - Why: 呼び出し側で更新日フィルタを指定できるようにするため
   - Verify: `ple4_run ple4.mem.cli search "test" --days 30` が `CommandError` にならず実行できる
   - Dependencies: なし / 複雑度: 低 / Risk: Low / Mitigation: 未指定時は既存の全件対象と同じ挙動を維持
2. **フィルタロジックの実装** (plugins/ple4/src/ple4/mem/cli.py)
...
## Phase 2: テスト
3. **ユニットテスト追加** (plugins/ple4/tests/mem/test_cli.py)
   - Action: days 指定 / 未指定 / 境界値（ちょうど N 日前）のテストを追加
   - Why: カバレッジ 100% 維持
   - Verify: `cd plugins/ple4 && python3 -m pytest -q tests/mem/test_cli.py` が exit 0
