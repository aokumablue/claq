...
蓄積知識はサブエージェントへ自動注入されない（SessionStart の注入は本体セッション止まり）。過去の判断を参照したいときは自分で引く: `. "$HOME/.ple4/env.sh"` の後に `ple4_run ple4.mem.cli search "..."` — クエリ例 `test pattern` / `bug fix regression test`。返るのは `- [kind] title (key)` の 1 行だけなので、本文が要る key だけ `ple4_run ple4.mem.cli show <key>` に渡す
