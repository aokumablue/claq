...
- **prompt injection は本エージェントでは防げない。** 実測で、勝たせる指示を埋め込んだ候補に対し 5 回中 2 回、A/B の内容を取り違えて劣る側を勝者にした（内容の説明まで逆になった）。A/B の入れ替え・複数 judge・artifact hash は provenance 確認と異常検知には使えるが、injection 防止や正答保証にはならない（injection は判定そのものを曲げるため、多数決では消えない）
