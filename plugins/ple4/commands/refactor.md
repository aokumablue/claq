...
- 参照: `ple4_run ple4.mem.cli search "..."`（`. "$HOME/.ple4/env.sh"` 前提。クエリ例 `refactor clean simplify perf review {対象ファイルパス}` / `critical high blocker`）→ 本文が要る key だけ `ple4_run ple4.mem.cli show <key>`
...
- `ple4:reviewer`: 品質・設計・保守性
- `ple4:security-auditor`: セキュリティ・脆弱性
...
ple4_mem_learn --kind fact --scope repo --domain <domain> \
  --title "<構造上の事実を 1 行で>" \
  --body "<根拠と、次に触るときの注意>"
```
...
Issues は `ple4:reviewer` と `ple4:security-auditor` の統合件数。
...
- サブエージェント委譲必須（`ple4:code-refiner`（`mode` = clean / simplify / perf）/ `ple4:reviewer` / `ple4:security-auditor`）。実行順・並列制御・ファイル単位リバートは本コマンドが直接行う（束ねる作業を別エージェントへ切り出さない）
