...
- 参照: `ple4_run ple4.mem.cli search "..."`（`. "$HOME/.ple4/env.sh"` 前提。クエリ例 `{棚卸し対象の domain}` / `{key}`）→ 本文が要る key だけ `ple4_run ple4.mem.cli show <key>`
...
ple4_run ple4.mem.cli <subcommand> [args...]
```
...
```bash
. "$HOME/.ple4/env.sh" || exit 127
ple4_run ple4.mem.cli list                          # 有効な知識の棚卸し
ple4_run ple4.mem.cli list --status pending         # 昇格待ちの候補（レビュー対象）
ple4_run ple4.mem.cli list --global --kind pitfall  # global の罠だけ
```
...
```bash
. "$HOME/.ple4/env.sh" || exit 127
ple4_mem_learn --key sqlite-wal-sidecars --kind pitfall --scope repo \
