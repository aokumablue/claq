...
**デシジョンテーブルの実装対象行が 0 行なら起動しない。** ベースラインが既に
全ブランチを到達している場合、引き継ぐタスク集合が空になり、起動しても
「重複テストを生成しない」（ルール節）に反する重複しか生まれない。この場合は
ステップ5 へ直行し、`Iterations` を `0/2 (loop-dev 未起動: 未到達ブランチ 0)`、
`Commits` を `none (変更なし)` と**注記**する（転記元が存在しないため転記ではない）。
`Gate` は `PASS` — 成果物ゼロでの正常完了であり、`BLOCKED` は
ステップ2 のテストコマンド検出失敗と、ステップ4 の外部ライブラリ追加確認待ちの
2 つに限る。

実装対象行が 1 行以上あるときは、続けて `loop-dev` skill を起動（必須）。`approved_plan` で plan 段を縮退（planner 省略）しつつ、generate→evaluate を最大 2 反復で収束させる。
