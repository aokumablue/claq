...
- 参照: `ple4_run ple4.mem.cli search "..."`（`. "$HOME/.ple4/env.sh"` 前提。クエリ例 `bug fix regression repro root cause verify` / `{対象ファイルパス}` / `{症状キーワード}`）→ 本文が要る key だけ `ple4_run ple4.mem.cli show <key>`
...
ステップ3で特定した**根本原因**を、次に同じ状況へ来る自分が回り道せずに済む形で残す。
...
ple4_mem_learn --kind pitfall --scope repo --domain <domain> \
  --title "<回避条件を 1 行で>" \
