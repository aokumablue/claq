...
- 参照: `ple4_run ple4.mem.cli search "..."`（`. "$HOME/.ple4/env.sh"` 前提。クエリ例 `harness audit score` / `harness config optimization audit` (days: 90)）→ 本文が要る key だけ `ple4_run ple4.mem.cli show <key>`
...
ple4_run ple4.ci.harness_audit <scope> --format <text|json> --root <path> --target-kind <repo|consumer>
```

スコアカードを出力。`--audit-only` は /harness レベルの制御フラグであり `ple4_run` へ渡さない。指定時はここで終了。`--root` / `--target-kind` はステップ4のベースライン比較まで同一値を保持する。
...
**apply 承認境界**: `--audit-only` は監査のみで完全に分離済みだが、`--audit-only` を指定しない既定の呼び出しでも、ここから先（ステップ3の変更適用）は無条件委譲しない。`--apply` が明示されていなければ、トップ3アクション（提案内容・対象ファイル・想定効果）をユーザーへ提示するだけで停止する（ステップ3〜5は実行しない）。`--apply` の明示がこの承認にあたる。`--apply` が無いまま harness-tuner へ変更適用を委譲しない。

## ステップ3: harness-tuner による改善適用（`--apply` 指定時のみ）

`ple4:harness-tuner` を起動。ベースラインJSONとトップ3アクションを渡し、信頼性・コスト・スループット最適化を委譲。
...
- `--audit-only`: ステップ1のみで終了（/harness レベルの制御フラグ。`ple4_run` へ渡さない）
