...
- 参照: `ple4_run ple4.mem.cli search "..."`（`. "$HOME/.ple4/env.sh"` 前提。クエリ例 `skill-gen pattern repository workflow`）→ 本文が要る key だけ `ple4_run ple4.mem.cli show <key>`
...
1. **`ple4:grader`**: 実行トランスクリプトと出力を期待値と照合し合否と根拠を整理
2. **`ple4:comparator`**: 改善前後（または2候補）の出力をブラインド比較しどちらが課題達成度が高いか判定
3. **`ple4:bench-analyzer`**: ベンチマーク結果と比較結果を要約し勝因・性能傾向を抽出
...
```bash
. "$HOME/.ple4/env.sh" || exit 127
ple4_mem_learn --key <slug> --kind convention --scope repo \
