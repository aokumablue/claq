...
リンク先は常に ``~/.grok/installed-plugins/ple4-*`` のみとする。
開発用チェックアウト（例: ple4-dev）は一切参照しない。
"""
...
def is_grok_installed_plugin_root(path: Path | str) -> bool:
    """パスが Grok の公式インストール先（installed-plugins/ple4-*）か判定する。

    Args:
        path: 検査対象パス。

    Returns:
        installed-plugins 配下の ple4-* ツリーなら True。
    ...
    if grok_idx + 2 >= len(parts) or parts[grok_idx + 1] != "installed-plugins":
        return False
    return parts[grok_idx + 2].startswith("ple4-")


def _has_launcher(root: Path) -> bool:
    """プラグインルートに launcher.py があるか。"""
    return (root / "src" / "ple4" / "launcher.py").is_file()
...
def _installed_target(path: Path | str) -> Path | None:
    """installed-plugins/ple4-* かつ launcher があるなら resolve した Path。"""
    candidate = Path(path)
...
def ensure_grok_plugin_root_symlink(
    *,
    ...
    """``~/.grok/plugins/ple4`` を **installed-plugins 配下のみ** へ symlink する。

    開発用リポジトリや任意パスはリンク先にしない。明示の ``plugin_root`` も
    ``is_grok_installed_plugin_root`` を満たす場合だけ採用する。

    Args:
        plugin_root: 候補のリンク先。installed-plugins/ple4-* でなければ無視。
        link_path: リンクのパス。None なら ``~/.grok/plugins/ple4``。
        home: ホーム上書き（テスト用）。
    ...
    """
    base = home if home is not None else _home()
    link = Path(link_path) if link_path is not None else base / ".grok" / "plugins" / "ple4"
...
def _resolve_symlink_target(plugin_root: Path | str | None, home: Path) -> Path | None:
    ...
    if env_root:
        ...
        if target is not None:
            return target
    found = find_latest_installed_ple4(installed_plugins_dir(home))
    if found is None:
        return None
