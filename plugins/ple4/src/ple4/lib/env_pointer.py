"""plugin root をベンダ非依存の固定住所（``~/.ple4/env.sh``）へ書き出す。
...
（``launcher.REPO_ROOT``）を ``~/.ple4`` 配下へ書き出す。md 側は
``. "$HOME/.ple4/env.sh"`` の 1 行だけを書けばよくなる。
...
- ポインタは shell script ではなく **データファイル**（絶対パス 1 行のみ）にする。
  ...
  ``PLE4_ROOT="..."`` という shell 代入形式は廃止した。
- ポインタの置き場所は ``get_ple4_dir()``（``PLE4_HOME`` を見る、
  テスト隔離用のノブ）ではなく **``$HOME/.ple4`` 固定**にする（R-04）。
  md の bootstrap 行 ``. "$HOME/.ple4/env.sh"`` は環境依存の分岐を持たない
  固定住所である必要があり、writer 側もそれに合わせる。データ永続化
  （``mem.db``・``logs``）側の ``get_ple4_dir()`` 契約とは別の契約であり、
  意図的に切り離している。
...
def _warn_once(reason: str) -> None:
    ...
    try:
        sys.stderr.write(
            json.dumps({"ple4EnvPointerWriteFailed": True, "reason": reason}) + "\n"
        )
    except OSError:
        pass
...
def _state_home() -> Path:
    ...
    ``PLE4_HOME`` は見ない（テスト隔離専用のノブであり、md の bootstrap 行が
    ``$HOME`` 固定であるため writer 側もそれに合わせる契約。データ永続化側の
    ``get_ple4_dir()``/``get_home_dir()`` とは別契約）。
...
def _ensure_private_dir(path: Path) -> Path:
    ...
    ``core_utils`` 側は ``get_ple4_dir()``（``PLE4_HOME`` 対応）基準で
...
def _maybe_run_gc(ple4_dir: Path, roots_dir: Path, *, keep_pids: frozenset[int]) -> None:
    ...
    Args:
        ...
        roots_dir: ``$HOME/.ple4/roots`` の Path。
    ...
    """
    stamp = ple4_dir / _GC_STAMP_FILENAME
    try:
