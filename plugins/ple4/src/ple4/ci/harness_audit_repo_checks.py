...
def _has_memory_lifecycle_hooks(root_dir: str | Path) -> bool:
    ...
    （``ple4.mem.cli context``、``ple4.mem.cli handoff``）を
    起動していることを確認する。
    ...
    required_commands = (
        ("SessionStart", (("ple4.mem.cli", "context"),)),
        ("SessionEnd", (("ple4.mem.cli", "handoff"),)),
    )
    return all(
...
def _hook_modules_resolve(root_dir: str | Path) -> bool:
    """hooks.json が参照する ple4 フックモジュールがすべて実在するかを判定する。
    ...
    if not raw:
        return False
    modules = set(re.findall(r"ple4\.hooks\.([A-Za-z_][A-Za-z0-9_]*)", raw))
    if not modules:
        return False
    return all(file_has_content(root_dir, f"src/ple4/hooks/{name}.py") for name in modules)
...
def _repo_tool_coverage_hooks_checks(root_dir: str | Path) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "fix": "Fix hooks.json entries that point at modules which do not exist under src/ple4/hooks/.",
        },
    ]
    ...
    return [
        {
            ...
            "fix": "Wire real memory lifecycle commands (ple4.mem.cli context/handoff) into "
        ...
        },
        {
            ...
            "points": 4,
            "scopes": ["repo", "hooks"],
            "path": "src/ple4/mem/cli.py",
            "description": "セッション永続化を担う mem CLI 実装が存在する",
            "pass": file_has_content(root_dir, "src/ple4/mem/cli.py"),
            "fix": "Implement src/ple4/mem/cli.py for session persistence (context/handoff commands).",
        },
        {
