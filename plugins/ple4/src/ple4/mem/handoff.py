...
from ple4.lib.core_utils import get_home_dir, strip_ansi
...
_TRANSCRIPT_ROOTS_ENV = "PLE4_TRANSCRIPT_ROOTS"
...
def _default_trusted_transcript_roots() -> tuple[Path, ...]:
    ...
    ``plugins/ple4/src`` 配下に host 検出コード（``COPILOT_*`` /
...
def _trusted_transcript_roots() -> tuple[Path, ...]:
    ...
    ``PLE4_TRANSCRIPT_ROOTS``（``os.pathsep`` 区切り）で追加・拡張できる
...
def trusted_transcript_roots() -> tuple[Path, ...]:
    ...
    Returns:
        既知 host の root と ``PLE4_TRANSCRIPT_ROOTS`` で追加された root。
