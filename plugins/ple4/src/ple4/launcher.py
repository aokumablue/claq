...
_USAGE = "Usage: python3 src/ple4/launcher.py [--bg] <module> [args...]"
...
def _unsupported_python_exit_code() -> int | None:
    ...
    （ple4 自体のモジュールは Python 3.12+ 前提の構文を使いうるため、
    ...
    sys.stderr.write(
        f"ERROR: ple4 requires Python 3.12+; `python3` is {version}. "
...
def _resolve_module_command(target: str, target_args: list[str]) -> list[str]:
    ...
    hooks.json / ple4-helpers.sh のターゲットはすべて dotted module
...
def main(argv: list[str] | None = None) -> int:
    ...
    from ple4.hooks.hook_common import detach_process, read_raw_stdin, write_stderr
    from ple4.lib.env_pointer import write_env_pointer

    # md（agents/commands/skills）がベンダ固有パスを書かずに plugin root を
    # 解決できるよう、全 hook 起動のたびに ~/.ple4/env.sh ポインタを更新する
