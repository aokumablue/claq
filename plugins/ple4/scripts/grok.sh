#!/bin/sh
# Grok Build TUI 用: ~/.grok/plugins/ple4 を ~/.grok/installed-plugins/ple4-<hash>
...
# ~/.grok/installed-plugins/ple4-* を直接探索して実体を見つける。
# 複数インストールが同居する場合は mtime 最新を選ぶ（Python 側の
# find_latest_installed_ple4 と同じ規則。両者が食い違うと、実行される
...
# 候補の選択規則は grok_plugin_root.find_latest_installed_ple4 と揃える
...
candidates = [
    path
    for path in (root.glob("ple4-*") if root.is_dir() else [])
    if (path / "src" / "ple4" / "lib" / "grok_plugin_root.py").is_file()
]
if not candidates:
    sys.exit(1)
print(max(candidates, key=lambda path: path.stat().st_mtime) / "src")
')" || {
    echo "no installed ple4-* found under ~/.grok/installed-plugins" >&2
    exit 1
}
...
from ple4.lib.grok_plugin_root import ensure_grok_plugin_root_symlink

result = ensure_grok_plugin_root_symlink()
if not result:
    print("could not link ~/.grok/plugins/ple4", file=sys.stderr)
    sys.exit(1)
