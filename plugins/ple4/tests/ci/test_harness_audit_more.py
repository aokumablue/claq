...
from ple4.ci.harness_audit_repo_checks import (
...
def test_find_plugin_install_and_build_report_variants(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    local_install = tmp_path / ".claude" / "plugins" / "ple4" / ".claude-plugin" / "plugin.json"
    ...
    (root / "agents").mkdir()
    (root / "skills").mkdir()
    (root / "src" / "ple4" / "ci").mkdir(parents=True)
    (root / "src" / "ple4" / "ci" / "harness_audit.py").write_text("", encoding="utf-8")
    (root / "package.json").write_text(json.dumps({"name": "ple4", "scripts": {"test": "x"}}), encoding="utf-8")
...
def test_find_plugin_install_without_home_still_searches_root(
    ...
    local_install = (
        tmp_path / ".claude" / "plugins" / "ple4" / ".claude-plugin" / "plugin.json"
    )
    local_install.parent.mkdir(parents=True, exist_ok=True)
    local_install.write_text("{}", encoding="utf-8")
    assert harness_audit.find_plugin_install(tmp_path) == str(local_install)
...
def test_main_entrypoint_exits_zero(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    (root / "package.json").write_text(
        json.dumps({"name": "ple4", "scripts": {"test": "x"}}),
        encoding="utf-8",
    )
    ...
    with pytest.raises(SystemExit) as excinfo:
        runpy.run_module("ple4.ci.harness_audit", run_name="__main__")
...
def test_get_repo_checks_python_structure_checks_pass_on_real_repo() -> None:
    ...
    ハーネス監査は元々 Node.js 実装(ple4)のルーブリックを
    ...
    （src/ple4/hooks/ 等）を対象にした変換後、実リポジトリに対して
    pass=True になることを確認する。

    tool-hooks-impl-count（最低12モジュール）・eval-tests-presence（最低60
    テストファイル）は、ple4 自身が過剰処理を廃止し src/tests を意図的に
...
def test_event_has_matching_command_covers_branches() -> None:
    ...
    patterns = (("ple4.mem.cli", "session:mem:setup"),)
