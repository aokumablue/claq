"""ple4.lib.grok_plugin_root のテスト。"""
...
def _make_installed_plugin(home: Path, name: str = "ple4-abc123") -> Path:
...
class TestIsGrokInstalledPluginRoot:
    ...
    def test_accepts_installed_plugins_ple4_hash(self, tmp_path: Path) -> None:
        """installed-plugins/ple4-* は True。"""
    ...
    def test_rejects_dev_checkout(self, tmp_path: Path) -> None:
        """開発用パスは False。"""
        dev = tmp_path / "dev" / "ple4-dev" / "plugins" / "ple4"
        (dev / "src" / "ple4").mkdir(parents=True)
        (dev / "src" / "ple4" / "launcher.py").write_text("#\n", encoding="utf-8")
        assert mod.is_grok_installed_plugin_root(dev) is False

    def test_rejects_grok_plugins_link_path(self, tmp_path: Path) -> None:
        """~/.grok/plugins/ple4 自体は installed ではない。"""
        path = tmp_path / ".grok" / "plugins" / "ple4"
    ...
    def test_rejects_grok_without_plugin_name(self, tmp_path: Path) -> None:
        """``.grok`` だけで installed-plugins/ple4-* が無いなら False。"""
        grok = tmp_path / ".grok"
        grok.mkdir()
...
class TestFindLatestInstalledPle4:
    ...
    def test_skips_without_launcher(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "ple4-empty").mkdir()
        assert mod.find_latest_installed_ple4(tmp_path) is None

    def test_missing_dir_returns_none(self, tmp_path: Path) -> None:
        """探索先が無ければ None。"""
        assert mod.find_latest_installed_ple4(tmp_path / "nope") is None

    def test_skips_non_ple4_and_files(self, tmp_path: Path) -> None:
        """ple4- 以外のディレクトリとファイルは無視する。"""
...
class TestEnsureGrokPluginRootSymlink:
    ...
    def test_ignores_dev_plugin_root_argument(self, tmp_path: Path) -> None:
        ...
        dev = tmp_path / "ple4-dev" / "plugins" / "ple4"
        (dev / "src" / "ple4").mkdir(parents=True)
        (dev / "src" / "ple4" / "launcher.py").write_text("#\n", encoding="utf-8")
        installed = _make_installed_plugin(tmp_path, "ple4-real")
        link = tmp_path / ".grok" / "plugins" / "ple4"
    ...
    def test_explicit_installed_plugin_root_used(self, tmp_path: Path) -> None:
        ...
        target = _make_installed_plugin(tmp_path, "ple4-explicit")
        _make_installed_plugin(tmp_path, "ple4-other")
        link = tmp_path / ".grok" / "plugins" / "ple4"
        result = mod.ensure_grok_plugin_root_symlink(
    ...
    def test_replaces_wrong_symlink(self, tmp_path: Path) -> None:
        ...
        wrong.mkdir()
        right = _make_installed_plugin(tmp_path, "ple4-right")
        link = tmp_path / ".grok" / "plugins" / "ple4"
    ...
    def test_plugin_root_resolve_oserror_falls_back(self, tmp_path: Path, monkeypatch) -> None:
        ...
        target = _make_installed_plugin(tmp_path, "ple4-explicit")
        real = _make_installed_plugin(tmp_path, "ple4-real")
        link = tmp_path / ".grok" / "plugins" / "ple4"
    ...
    def test_env_root_without_launcher_falls_back(self, tmp_path: Path, monkeypatch) -> None:
        ...
        empty = tmp_path / ".grok" / "installed-plugins" / "ple4-empty"
    ...
    def test_uses_claude_plugin_root_env(self, tmp_path: Path, monkeypatch) -> None:
        ...
        target = _make_installed_plugin(tmp_path, "ple4-env")
        _make_installed_plugin(tmp_path, "ple4-other")
    ...
    def test_env_root_resolve_oserror_falls_back(self, tmp_path: Path, monkeypatch) -> None:
        ...
        env_target = _make_installed_plugin(tmp_path, "ple4-env")
        real = _make_installed_plugin(tmp_path, "ple4-real")
    ...
    def test_leaves_existing_launcher_tree(self, tmp_path: Path) -> None:
        ...
        _make_installed_plugin(tmp_path)
        link = tmp_path / ".grok" / "plugins" / "ple4"
        launcher = link / "src" / "ple4" / "launcher.py"
        launcher.parent.mkdir(parents=True, exist_ok=True)
        launcher.write_text("#\n", encoding="utf-8")
        assert mod.ensure_grok_plugin_root_symlink(home=tmp_path, link_path=link) is None
