...
def _isolate_ple4_home(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """main() 経由の write_env_pointer が実 HOME の ~/.ple4 を汚さないようにする。

    main() は全 hook 起動で ~/.ple4/env.sh 等を書き出す（M-02 対応）。
    env_pointer の state dir は ``$HOME`` 固定（``PLE4_HOME`` は見ない。R-04 —
    md の bootstrap 行 `. "$HOME/.ple4/env.sh"` が固定住所であるべきという
...
class TestUnsupportedPythonExitCode:
    ...
    def test_returns_zero_on_old_python_and_writes_stderr(
        ...
        assert lines[0] == (
            f"ERROR: ple4 requires Python 3.12+; `python3` is {displayed}. "
            "Point `python3` on PATH at 3.12+ (ple4 does not create a venv)."
        )
        payload = json.loads(lines[1])
        assert payload == {
            "ple4ProtectionDisabled": True,
...
class TestRunModuleInProcess:
    ...
    def test_system_exit_zero_returns_zero(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert launcher._run_module_in_process("ple4.hooks.block_no_verify", []) == 0
    ...
    def test_sets_argv_from_target_and_args(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        launcher._run_module_in_process("ple4.mem.cli", ["setup"])

        assert captured["argv"] == ["ple4.mem.cli", "setup"]
...
class TestResolveModuleCommand:
    ...
    def test_builds_module_invocation(self) -> None:
        ...
        cmd = launcher._resolve_module_command("ple4.hooks.config_protection", ["a", "b"])
        assert cmd == [sys.executable, "-m", "ple4.hooks.config_protection", "a", "b"]

    def test_builds_module_invocation_without_args(self) -> None:
        """引数なしならモジュール名だけを付ける。"""
        cmd = launcher._resolve_module_command("ple4.mem.cli", [])
        assert cmd == [sys.executable, "-m", "ple4.mem.cli"]
...
class TestMain:
    ...
    def test_normal_invocation_runs_in_process(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert launcher.main(["ple4.hooks.config_protection", "extra"]) == 0
        assert captured == {"target": "ple4.hooks.config_protection", "args": ["extra"]}

    def test_bg_always_detaches_and_returns_zero(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """--bg は host に関わらず常に detach して 0 を返す。"""
        monkeypatch.setattr("ple4.hooks.hook_common.read_raw_stdin", lambda: "{}")
        ...
        monkeypatch.setattr("ple4.hooks.hook_common.detach_process", fake_detach)
        ...
        assert launcher.main(["--bg", "ple4.mem.cli", "handoff"]) == 0
        assert run_in_process_called == []
        assert captured["cmd"] == [sys.executable, "-m", "ple4.mem.cli", "handoff"]
        assert captured["raw"] == "{}"
    ...
    def test_bg_detach_failure_returns_nonzero(
        ...
        """
        monkeypatch.setattr("ple4.hooks.hook_common.read_raw_stdin", lambda: "")
        monkeypatch.setattr("ple4.hooks.hook_common.detach_process", lambda *a, **k: False)
        monkeypatch.setattr(launcher, "build_env", lambda: {})

        assert launcher.main(["--bg", "ple4.mem.cli", "handoff"]) == 1
    ...
    def test_entrypoint_returns_usage_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """``python3 -m ple4.launcher`` 相当は引数なしで usage 終了する。"""
        monkeypatch.setattr(sys, "argv", ["launcher.py"])

        with pytest.raises(SystemExit) as excinfo:
            runpy.run_module("ple4.launcher", run_name="__main__")
