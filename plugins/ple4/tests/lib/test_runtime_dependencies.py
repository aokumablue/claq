"""ple4 本体がランタイムでサードパーティ依存を持たないことを機械的に保証するテスト。

`src/ple4/**/*.py` を `ast` で走査し、全 import 文のトップレベルモジュール名が
...
PACKAGE_ROOT = Path(__file__).resolve().parents[2] / "src" / "ple4"

# CLAUDE.md / `launcher.py` が宣言する対応 Python の下限。
MIN_PYTHON = (3, 12)

# ple4 が import してよいトップレベルモジュール名。
#
# `ple4` 以外はすべて Python 3.12 の標準ライブラリである（最も新しい `tomllib`
...
def _iter_source_files() -> list[Path]:
    """走査対象となる ple4 パッケージ配下の Python ソースを列挙する。"""
...
def _imported_top_level_modules() -> frozenset[str]:
    """ple4 パッケージ全体が import しているトップレベル名の集合を返す。"""
...
class TestRuntimeDependencies:
    ...
    def test_allowlist_entries_exist_in_running_stdlib(self) -> None:
        ...
        """
        missing = sorted((ALLOWED_TOP_LEVEL_MODULES - {"ple4"}) - set(sys.stdlib_module_names))
    ...
    def test_allowlist_excludes_stdlib_added_after_minimum(self) -> None:
        ...
        """
        assert _STDLIB_ADDED_AFTER_MIN_PYTHON <= set(sys.stdlib_module_names), (
            "対照サンプルが実行中インタプリタの標準ライブラリに無い。"
            "旧実装が素通りさせていた条件を再現できないため、サンプルを見直すこと"
        )
