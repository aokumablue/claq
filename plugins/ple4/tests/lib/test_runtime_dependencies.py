...
`src/ple4/**/*.py` を `ast` で走査し、全 import 文のトップレベルモジュール名から
標準ライブラリ（`sys.stdlib_module_names`）と自己参照（`ple4`・相対 import）を
差し引いた集合が空であることを表明する。
...
ALLOWED_TOP_LEVEL_MODULES = frozenset(sys.stdlib_module_names) | {"ple4"}
...
class TestRuntimeDependencies:
    ...
    def test_no_third_party_runtime_imports(self) -> None:
        """ple4 パッケージが標準ライブラリと自身以外を import しないこと。"""
        violations: list[str] = []
        ...
        assert not violations, (
            "ple4 はランタイム依存ゼロ（標準ライブラリのみ）である必要があります。"
