...
_SCRIPT = _REPO_ROOT / "plugins" / "ple4" / "scripts" / "grok.sh"
...
def test_no_installed_plugin_exits_nonzero_without_link(tmp_path: Path) -> None:
    ...
    link = tmp_path / ".grok" / "plugins" / "ple4"
    assert "no installed ple4-* found" in result.stderr
...
def test_picks_newest_install_like_the_python_resolver(tmp_path: Path) -> None:
    ...
    """
    older = _make_installed_plugin(tmp_path, name="ple4-aaa111")
    newer = _make_installed_plugin(tmp_path, name="ple4-zzz999")
