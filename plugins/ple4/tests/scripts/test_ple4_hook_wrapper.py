...
デシジョンテーブル（POSIX 側）:
  ...
  - PATH に絶対エントリが 1 つも無い  → 起動せず exit 0 + 診断（PLE4_PYTHON より前）
  - PLE4_PYTHON が絶対パス          → そのインタプリタで launcher を起動
  - PLE4_PYTHON が相対名/相対パス    → 起動せず exit 0 + 診断（PATH 探索へ落とさない）
...
def test_wrapper_fails_open_when_launcher_is_missing(tmp_path: Path) -> None:
    ...
    `PLE4_PYTHON` に**実在するインタプリタ**を渡しているのが要点。使えない
    ...
    assert str(tmp_path / "src" / "ple4" / "launcher.py") in result.stderr.splitlines()[0]
...
def test_wrapper_fails_open_when_launcher_is_unreadable(tmp_path: Path) -> None:
    ...
    launcher = tmp_path / "src" / "ple4" / "launcher.py"
    ...
    try:
        result = _run_wrapper("ple4.hooks.pre_compact", env=env, wrapper=copy)
    finally:
...
def test_launcher_diagnostics_never_embed_a_path_in_json() -> None:
    ...
    （`{"ple4ProtectionDisabled": false}` を後続行として差し込む等）。移植可能な
    エスケープ手段が無い（`${var//}` は bash 専用、`sed` は PATH が壊れている
    この状況では呼べない）ので、パスは人間可読行だけに載せる。Windows 側も
    同じ理由で `%PLE4_LAUNCHER_JSON%` ごと落としてある（片側だけ塞がない）。
    """
    ...
    # （`"candidates": ["PLE4_PYTHON", ...]` は固定文字列なので安全）。
    for line in posix_json:
        assert "$_ple4_launcher" not in line
        assert "$PLE4_PYTHON" not in line
        assert "${PLE4_PYTHON" not in line
    for line in windows_json:
        assert "%PLE4_LAUNCHER" not in line
        assert "%PLE4_PYTHON%" not in line
    # 使われなくなった JSON 用のパス変数ごと消えていること。
    assert "PLE4_LAUNCHER_JSON" not in windows


def test_wrapper_uses_ple4_python_override(tmp_path: Path) -> None:
    """PLE4_PYTHON（絶対パス）が最優先で使われ、launcher と対象モジュールまで到達する。"""
    empty_bin = tmp_path / "empty-bin"
    empty_bin.mkdir()
    env = _isolated_env(tmp_path, path_dirs=[empty_bin])
    env["PLE4_PYTHON"] = sys.executable
...
def test_wrapper_probe_does_not_consume_the_hook_payload(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [str(_WRAPPER), "ple4.hooks.pre_compact"],
...
def test_wrapper_reports_when_no_interpreter_exists(tmp_path: Path) -> None:
    ...
    result = _run_wrapper("ple4.hooks.pre_compact", env=_isolated_env(tmp_path, path_dirs=[empty_bin]))
...
def test_wrapper_never_adopts_a_cwd_interpreter(
    ...
    result = _run_wrapper("ple4.hooks.pre_compact", env=env, cwd=repo)
...
def test_wrapper_exports_a_sanitized_path_to_the_interpreter(tmp_path: Path) -> None:
    ...
    result = _run_wrapper("ple4.hooks.pre_compact", env=env, cwd=tmp_path)
...
def test_wrapper_rejects_a_relative_ple4_python(tmp_path: Path, label: str, value: str) -> None:
    """PLE4_PYTHON が相対なら cwd のインタプリタを絶対に起動しないこと。

    `PLE4_PYTHON=./python3` は POSIX 側でも実測で乗っ取りに成立した（監査は
    ...
    assert _diagnostic(result)["reason"] == "ple4_python_not_absolute", label
...
def test_windows_wrapper_checks_the_launcher_before_choosing_an_interpreter() -> None:
    ...
    readable = next(i for i, line in enumerate(lines) if line.startswith('type "%PLE4_LAUNCHER%"'))
    ...
    assert lines[readable + 1] == "if errorlevel 1 goto :ple4_no_launcher"
    assert lines[guard].endswith("goto :ple4_no_launcher")
...
def test_windows_wrapper_detects_empty_path_entries() -> None:
    ...
    **このテストは形だけを固定する。** darwin から cmd.exe を実行できないため、
    ...
    # 無効化する。`ple4-hook.cmd` の KNOWN RESIDUAL と docs/adr/01-hook-failure-direction.md を参照。
...
def test_windows_wrapper_requires_an_absolute_ple4_python() -> None:
    """PLE4_PYTHON の絶対パス検査が Windows 側にもあること（B）。

    cmd.exe は裸の名前も `.\\name` も**常に cwd から先に**解決するため、この穴は
    Windows では無条件に成立する。POSIX 側は `PLE4_PYTHON=./python3` が実測で
    ...
    """
    lines = _cmd_lines()
    override = next(i for i, line in enumerate(lines) if line == ":ple4_override")
    exec_label = next(i for i, line in enumerate(lines) if line == ":ple4_exec")
    gates = [line for line in lines if line.startswith('if "%PLE4_PYTHON:~')]

    assert len(gates) == 4
    accepted = set()
    for gate in gates:
        assert gate.endswith("goto :ple4_override_ok")
    ...
    reject = next(i for i, line in enumerate(lines) if "ple4_python_not_absolute" in line)
    assert lines[reject + 1] == "exit /b 0"
    assert not any(line.startswith("call :ple4_pick") for line in lines[override:])


def test_windows_wrapper_quotes_every_path_expansion() -> None:
    """パスを持ちうる %VAR% が必ず引用符の内側で展開されること（C）。

    cmd はパーセント展開の**後**に行を再パースするので、`echo ... %PLE4_LAUNCHER%`
    のような裸の展開はインストールパスに `&` があるとコマンド実行になる。
    Windows のアカウント名は `&` を許すため `C:\\Users\\A&B\\...` は攻撃者が
    いなくても成立する。
    """
    for line in _cmd_lines():
        for var in ("%PLE4_LAUNCHER%", "%PLE4_PYTHON%", "%PLE4_PY%", "%PLE4_ROOT%"):
            start = line.find(var)
            while start != -1:
...
def test_windows_wrapper_probes_python3_as_well() -> None:
    ...
    # probe が `:ple4_pick` の唯一の出口条件であること（候補名で分岐しない）。
    pick = lines[lines.index(":ple4_pick") :]
    early_exits = [line for line in pick if line.startswith("if not defined PLE4_PY exit /b")]
    assert early_exits == ["if not defined PLE4_PY exit /b 0"]
