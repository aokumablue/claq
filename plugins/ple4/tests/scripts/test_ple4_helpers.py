...
_HELPER = _REPO_ROOT / "plugins" / "ple4" / "runtime" / "ple4-helpers.sh"
_PLUGIN_ROOT = _REPO_ROOT / "plugins" / "ple4"
...
def _print_plugin_root_script(*, unset_env: bool = False, cd_to: Path | None = None) -> str:
    """helper を source して ple4_plugin_root を 1 行出す。
...
def _print_plugin_root_script_relative_source_then_cd(cd_to: Path) -> str:
    ...
    _REPO_ROOT で相対パス "plugins/ple4/runtime/ple4-helpers.sh" を
    source し、CLAUDE_PLUGIN_ROOT を unset した状態で cd_to へ cd してから
    ple4_plugin_root を呼ぶ。fix 前のコードは BASH_SOURCE[0] を
    ple4_plugin_root 呼び出し時に相対パスのまま dirname 解決していたため、
    source 後に cd すると cwd 起点で解決が壊れる（bash でも zsh でも）。
    fix 後のコードは source 時点でファイル先頭スコープの
    `_PLE4_HELPERS_DIR` を絶対パスへ解決済みのため、この cd の影響を受けない。
    """
    return f'''
set -euo pipefail
cd "{_REPO_ROOT}"
...
def test_ple4_plugin_root_relative_source_then_cd_survives_under_bash(tmp_path: Path) -> None:
...
@_skip_without_zsh
def test_ple4_plugin_root_relative_source_then_cd_survives_under_zsh(tmp_path: Path) -> None:
...
def _print_root_with_handover(root: Path) -> str:
    """env.sh が渡す _PLE4_SOURCED_ROOT 経由で root を出力するスクリプトを返す。"""
    return "\n".join(
        [
            "set -eu",
...
def test_ple4_plugin_root_ignores_ambient_claude_plugin_root(tmp_path: Path) -> None:
...
def test_ple4_plugin_root_uses_sourced_root_handed_over_by_env_sh(tmp_path: Path) -> None:
    """env-template.sh が渡す _PLE4_SOURCED_ROOT を root として使うこと。
...
def test_ple4_plugin_root_uses_file_location_fallback_without_env() -> None:
...
@_skip_without_zsh
def test_ple4_plugin_root_uses_file_location_fallback_with_env_under_zsh(tmp_path: Path) -> None:
...
@_skip_without_zsh
def test_ple4_plugin_root_uses_file_location_fallback_without_env_under_zsh(tmp_path: Path) -> None:
