...
from ple4.lib.harness import _TOOL_NAME_MAP, normalize_tool_name
...
TOOL_GATED_HOOKS: dict[str, frozenset[str]] = {
    ...
    "ple4.hooks.block_no_verify": frozenset({"Bash"}),
    "ple4.hooks.pre_bash_commit_quality": frozenset({"Bash"}),
    # _BASH_TOOL_NAMES == {bash}（正規化後 "Bash"）。config_protection の
    # Edit/Write/MultiEdit 限定を補完する Bash 経由の直接書き換え検知（A-06）。
    "ple4.hooks.bash_config_protection": frozenset({"Bash"}),
    # _WRITE_TOOL_NAMES == {edit, write, multiedit}
    "ple4.hooks.config_protection": frozenset({"Edit", "Write", "MultiEdit"}),
}
