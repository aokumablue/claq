...
from ple4.hooks.bash_config_protection import _BASH_TOOL_NAMES
from ple4.hooks.config_protection import _WRITE_TOOL_NAMES
from ple4.lib.harness import INPUT_CONTAINER_KEYS, normalize_tool_name
...
_ISOLATED_ENV_KEYS = frozenset({
    ...
    "PLUGIN_DATA",
    "PLE4_HOME",
    # mem のデータ位置を決めるのはこちら（src/ple4/mem/settings.py）。
    # PLE4_HOME は lib/core_utils.get_ple4_dir() が読む別変数で、
    # これを消しても実 ~/.ple4 への書き込みは止まらなかった。
    "PLE4_DATA_PATH",
    "CLAUDE_SESSION_ID",
    "CLAUDE_PROJECT_DIR",
    "CLAUDE_PLUGIN_ROOT",
...
def _launcher_env(tmp_home: Path) -> dict[str, str]:
    ...
    Args:
        tmp_home: 一時 HOME。実ユーザーの ~/.ple4 を触らない。
...
def _run_launcher(hook: str, payload: dict, tmp_home: Path) -> subprocess.CompletedProcess[str]:
    ...
    Args:
        hook: dotted module name（例: ple4.hooks.config_protection）。
...
def test_declared_hook_target_is_importable(event: str, args: list[str], timeout: int) -> None:
    ...
    """
    module = next(arg for arg in args if arg.startswith("ple4."))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ple4.hooks.block_no_verify", "git commit --no-verify -m x", True),
        ("ple4.hooks.block_no_verify", "git commit -n -m x", True),
        ("ple4.hooks.block_no_verify", "git commit -m x", False),
        ("ple4.hooks.block_no_verify", "git log -n 5", False),
        # bash_config_protection: リンタ設定の弱体化
        ("ple4.hooks.bash_config_protection", "echo x > ruff.toml", True),
        ("ple4.hooks.bash_config_protection", "rm .eslintrc", True),
        ("ple4.hooks.bash_config_protection", "sudo cp /tmp/x ruff.toml", True),
        ("ple4.hooks.bash_config_protection", "cat ruff.toml", False),
        ("ple4.hooks.bash_config_protection", "ls -la", False),
        # pre_bash_commit_quality: commit 以外は素通り
        ("ple4.hooks.pre_bash_commit_quality", "ls -la", False),
    ],
)
...
def test_session_start_emits_valid_json_and_creates_db(tmp_path: Path) -> None:
    ...
    """
    result = subprocess.run(
        [sys.executable, str(LAUNCHER), "ple4.mem.cli", "context"],
    ...
    assert (tmp_path / ".ple4" / "mem.db").is_file()
...
_SHAPE_GATED_HOOKS = {
    "ple4.hooks.block_no_verify": ("Bash", "command", "git commit --no-verify -m x", "git commit -m x"),
    "ple4.hooks.bash_config_protection": ("Bash", "command", "echo x > ruff.toml", "cat sample.py"),
    "ple4.hooks.config_protection": ("Write", "file_path", "ruff.toml", "sample.py"),
}
...
def _pre_tool_use_modules() -> set[str]:
    ...
    return {
        next(arg for arg in args if arg.startswith("ple4."))
...
_MULTIPLICITY_GATED_HOOKS = {
    "ple4.hooks.block_no_verify": ("Bash", "command", "git commit --no-verify -m x", "echo safe"),
    "ple4.hooks.bash_config_protection": ("Bash", "command", "echo x > ruff.toml", "echo safe"),
    "ple4.hooks.config_protection": ("Write", "file_path", "ruff.toml", "sample.py"),
    "ple4.hooks.pre_bash_commit_quality": ("Bash", "command", "git add . && git commit -m x", "echo safe"),
}
...
_IN_HOOK_TOOL_NAME_GATES = {
    "ple4.hooks.bash_config_protection": _BASH_TOOL_NAMES,
    "ple4.hooks.config_protection": _WRITE_TOOL_NAMES,
}
...
#
# 検出は symbol 直 import に限る。`from ple4.lib import harness` +
...
def _harness_imports(module: str) -> set[str]:
    ...
    return {
        ...
        if isinstance(node, ast.ImportFrom) and node.module == "ple4.lib.harness"
...
_FIELD_MULTIPLICITY_GATED_HOOKS = {
    "ple4.hooks.block_no_verify": ("Bash", "command", "echo safe", "cmd", "git commit --no-verify -m x"),
    "ple4.hooks.bash_config_protection": ("Bash", "command", "echo safe", "cmd", "echo x > ruff.toml"),
    "ple4.hooks.pre_bash_commit_quality": ("Bash", "command", "echo safe", "cmd", "git add . && git commit -m x"),
    "ple4.hooks.config_protection": ("Write", "file_path", "sample.py", "file", "ruff.toml"),
}
...
def _harness_source() -> str:
    """`ple4.lib.harness` のソーステキストを返す。

    Returns:
        harness モジュールのソース全文。
    """
    return Path(importlib.util.find_spec("ple4.lib.harness").origin).read_text(encoding="utf-8")
