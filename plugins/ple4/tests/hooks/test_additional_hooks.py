...
class TestBlockNoVerify:
    ...
    def test_end_to_end_via_module_subprocess(self, command: str, expected_code: int) -> None:
        ...
        completed = subprocess.run(
            [sys.executable, "-m", "ple4.hooks.block_no_verify"],
            input=payload,
            capture_output=True,
            text=True,
            check=False,
...
class TestPreCompact:
    ...
    def test_main_entrypoint_exits_zero(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        monkeypatch.setattr("ple4.lib.core_utils.get_sessions_dir", lambda: tmp_path)
        monkeypatch.setattr("ple4.lib.core_utils.ensure_dir", lambda path: Path(path))
        monkeypatch.setattr("ple4.lib.core_utils.append_file", lambda path, content: None)
        monkeypatch.setattr("ple4.lib.core_utils.get_datetime_string", lambda: "2026-01-01 00:00:00")
        monkeypatch.setattr("ple4.lib.core_utils.log", lambda message: None)

        assert _run_entrypoint("ple4.hooks.pre_compact") == 0
...
class TestSimpleHookEntrypoints:
    def test_block_no_verify_entrypoint_exits_zero(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        assert _run_entrypoint("ple4.hooks.block_no_verify") == 0
...
def test_empty_input_passthrough(monkeypatch: pytest.MonkeyPatch, module_name: str) -> None:
    ...
    mod = importlib.import_module(f"ple4.hooks.{module_name}")
