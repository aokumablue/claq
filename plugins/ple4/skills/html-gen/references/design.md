...
- `svg[role="img"]` には**例外なく** `<title>` と `<desc>`（`aria-labelledby` で束ねる）。
  スパークラインのような小さな図も対象で、`aria-label` 1 つで済ませない
- 数値への到達手段を図の外にも用意する。各データ点の `<title>`（ホバーで読める）、
  `<desc>` の要点、そしてページに少なくとも 1 つの数値表。**図ごとに表を複製する
  必要は無い** — ダッシュボードでページが倍になる。読み物寄りのページで図の全数値が
  要るなら `<details>` に畳んだ表を図の直下へ置く
