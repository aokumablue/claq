...
- `stop_reason` / `remaining_turns` は反復ループが中断・停止した際に記録する任意フィールド。ループ対象外の一般チェックポイントや `completed: true` では、フィールドを残して値を `-` にする。`remaining_turns` は再開時に消費可能な残り反復数の目安値
...
- blocker シグネチャ: `正規化相対パス~指摘要旨先頭8語` — 小文字化・行番号除去・数値マスクを適用した後の先頭8語
...
### 完了マーク
タスク完了時は `completed: true` に変更し、「進行中」セクションを空にする。`completed: true` の checkpoint は次セッションへ自動注入されない。
