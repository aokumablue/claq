...
なる」という事故が起きる（v0.9.52 監査 H-3。実測で再現済み）。既存 `status` を維持
したまま本文だけ更新する案も検討したが、それでは「人間が承認した内容と違うものが
注入され続ける」という別の穴が開くため採らなかった。**`active` カードは agent の
書込みに対して不変**、という 1 行の不変条件にしてある。
