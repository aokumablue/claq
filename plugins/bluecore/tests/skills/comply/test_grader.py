...
from bluecore.skills.comply.grader import ComplianceResult, grade
from bluecore.skills.comply.parser import ComplianceSpec, ObservationEvent, parse_spec, parse_trace

FIXTURES = (
    Path(__file__).resolve().parents[3] / "src" / "bluecore" / "skills" / "comply" / "fixtures"
)
...
class TestGradeCompliant:
    @patch("bluecore.skills.comply.grader.classify_events", side_effect=_mock_compliant_classification)
...
class TestGradeNoncompliant:
    @patch("bluecore.skills.comply.grader.classify_events", side_effect=_mock_noncompliant_classification)
...
class TestGradeEdgeCases:
    @patch("bluecore.skills.comply.grader.classify_events", side_effect=_mock_empty_classification)
...
class TestTemporalOrderAfterStepViolation:
    ...
    def _make_event(self, ts: str, tool: str = "Bash") -> ObservationEvent:
        """指定タイムスタンプの観測イベントを返す。"""
        from bluecore.skills.comply.parser import ObservationEvent
    ...
    @patch("bluecore.skills.comply.grader.classify_events")
...
def test_check_temporal_order_before_step_resolved_empty() -> None:
    ...
    from bluecore.skills.comply.grader import _check_temporal_order
    from bluecore.skills.comply.parser import Detector, Step
