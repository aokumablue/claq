"""bluecore.skills.stocktake.io のテスト。"""
...
from bluecore.skills.stocktake import io as sio
...
def test_atomic_write_cleans_up_temp_on_error(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("bluecore.skills.stocktake.io.os.replace", _bad_replace)
...
def test_atomic_write_unlink_error_is_suppressed(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("bluecore.skills.stocktake.io.os.replace", _bad_replace)
    monkeypatch.setattr("bluecore.skills.stocktake.io.os.unlink", _bad_unlink)
