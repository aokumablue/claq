"""bluecore.skills.stocktake.cli のテスト。"""
...
from bluecore.skills.stocktake import cli as _mod
...
def test_cmd_diff_no_changes_outputs_empty_array(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture) -> None:
    ...
    import bluecore.skills.stocktake.core as core_mod
    ...
    monkeypatch.setattr("bluecore.skills.stocktake.cli.core.classify_changed", _patched_classify)
...
def test_cmd_save_creates_results_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with patch("bluecore.skills.stocktake.cli.datetime") as mock_dt:
...
def test_entrypoint_exits_nonzero_on_no_args(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(SystemExit) as exc:
        runpy.run_module("bluecore.skills.stocktake.cli", run_name="__main__")
