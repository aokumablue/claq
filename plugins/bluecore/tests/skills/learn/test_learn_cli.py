...
from bluecore.skills.learn import cli as _mod
...
def project_tree(tmp_path):
    ...
    projects_dir = bluecore_dir / "projects"
    global_personal = bluecore_dir / "instincts" / "personal"
    global_inherited = bluecore_dir / "instincts" / "inherited"
    global_evolved = bluecore_dir / "evolved"
...
def patch_globals(project_tree, monkeypatch):
    ...
    monkeypatch.setattr(_mod, "BLUECORE_DIR", project_tree["bluecore"])
    ...
    monkeypatch.setattr(_mod, "GLOBAL_INSTINCTS_DIR", project_tree["bluecore"] / "instincts")
    ...
    monkeypatch.setattr(_mod, "GLOBAL_OBSERVATIONS_FILE", project_tree["bluecore"] / "observations.jsonl")
    return project_tree
...
def test_cmd_prune_dry_run_and_delete(patch_globals, monkeypatch, capsys):
    tree = patch_globals
    monkeypatch.setattr(_mod, "GLOBAL_INSTINCTS_DIR", tree["bluecore"] / "instincts")
    project = _make_project(tree)
...
def test_import_handles_missing_fcntl_and_ensure_global_dirs(patch_globals, monkeypatch):
    ...
    monkeypatch.setattr(_mod, "BLUECORE_DIR", patch_globals["bluecore"])
    monkeypatch.setattr(_mod, "GLOBAL_INSTINCTS_DIR", patch_globals["bluecore"] / "instincts")
    ...
    monkeypatch.setattr(_mod, "GLOBAL_OBSERVATIONS_FILE", patch_globals["bluecore"] / "observations.jsonl")
...
def test_main_module_entrypoint_uses_sys_exit(tmp_path, monkeypatch):
    ...
    with pytest.raises(SystemExit) as excinfo:
        runpy.run_module("bluecore.skills.learn.cli", run_name="__main__")
...
def test_all_project_dirs_when_projects_dir_missing(monkeypatch, tmp_path) -> None:
    ...
    import bluecore.skills.learn.cli.paths as p
...
def test_parse_frontmatter_line_without_colon() -> None:
    """コロンが無い行は無視する。"""
    from bluecore.skills.learn.cli.instincts import _parse_frontmatter_line
...
def test_extract_created_frontmatter_ends_without_created() -> None:
    ...
    from bluecore.skills.learn.cli.pending import _extract_created_from_frontmatter
...
def test_print_instincts_by_domain_without_action(capsys) -> None:
    ...
    from bluecore.skills.learn.cli.instincts import _print_instincts_by_domain
...
def test_print_skill_candidates_empty() -> None:
    """スキル候補が空なら何も出力しない。"""
    from bluecore.skills.learn.cli.evolve import _print_skill_candidates
...
def test_print_agent_candidates_empty() -> None:
    """エージェント候補が空なら何も出力しない。"""
    from bluecore.skills.learn.cli.evolve import _print_agent_candidates
...
def test_resolve_project_root_git_nonzero(monkeypatch) -> None:
    ...
    import bluecore.skills.learn.cli.registry as r
...
def test_show_promotion_candidates_low_confidence(patch_globals, capsys):
    ...
    import bluecore.skills.learn.cli.evolve as ev
...
def test_print_pending_summary_few_and_none_expiring(capsys):
    ...
    import bluecore.skills.learn.cli.commands as cmds
