...
from bluecore.lib.install.install_state import (
    InstallOperation,
    InstallRequest,
    InstallSource,
    InstallState,
...
def make_valid_state() -> dict:
    ...
    return {
        "schemaVersion": "bluecore.install.v1",
...
class TestInstallResolutionModel:
    ...
    def test_rejects_empty_string_in_selected_modules(self):
        with pytest.raises(ValidationError):
            from bluecore.lib.install.install_state import InstallResolution
...
class TestInstallStateModel:
    ...
    def test_valid_state(self):
        ...
        assert state.schemaVersion == "bluecore.install.v1"
...
class TestReadInstallState:
    ...
    def test_reads_valid_file(self, tmp_path):
        ...
        assert result["schemaVersion"] == "bluecore.install.v1"
    ...
    def test_raises_for_invalid_labelled_state(self, tmp_path):
        state_file = tmp_path / "state.json"
        state_file.write_text(json.dumps({"schemaVersion": "bluecore.install.v1"}), encoding="utf-8")
