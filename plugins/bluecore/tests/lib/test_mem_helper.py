...
from bluecore.lib.mem_helper import (
    RecordEventParams,
    _truncate,
...
class TestSearchSimilarContext:
    ...
    @patch("bluecore.lib.mem_helper._run_mem_cli")
...
class TestGetProjectStats:
    ...
    @patch("bluecore.lib.mem_helper.time.time", return_value=1704067200)
    @patch("bluecore.lib.mem_helper.Database")
    @patch("bluecore.lib.mem_helper.Settings.load")
    @patch("bluecore.lib.mem_helper.Path.cwd")
...
class TestRunMemCliIntegration:
    ...
    def test_invalid_command(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from bluecore.lib.mem_helper import _run_mem_cli
...
class TestRunMemCliPaths:
    ...
    def test_success_path(self, monkeypatch) -> None:
        """正常終了時はJSONをパースして返す"""
        from bluecore.lib import mem_helper
...
def test_format_context_for_prompt_without_content() -> None:
    ...
    from bluecore.lib.mem_helper import format_context_for_prompt
