...
def test_no_installed_plugin_exits_nonzero_without_link(tmp_path: Path) -> None:
    ...
    link = tmp_path / ".grok" / "plugins" / "bluecore"
    assert "no installed bluecore-* found" in result.stderr
...
def test_picks_newest_install_like_the_python_resolver(tmp_path: Path) -> None:
    ...
    """
    older = _make_installed_plugin(tmp_path, name="bluecore-aaa111")
    newer = _make_installed_plugin(tmp_path, name="bluecore-zzz999")
