...
_HOOK_BODIES = {
    "bluecore.hooks.block_no_verify": "git commit --no-verify -m x",
    "bluecore.hooks.bash_config_protection": "printf x > ruff.toml",
}
...
def _run(hook: str, command: str, tmp_home: Path, cwd: Path) -> subprocess.CompletedProcess[str]:
    ...
    Args:
        ...
        tmp_home: 一時 HOME（実 ~/.bluecore を触らない）。
...
def test_commit_quality_does_not_scan_for_data_heredoc_body(operator_line: str, tmp_path: Path, repo: Path) -> None:
    ...
    result = _run("bluecore.hooks.pre_bash_commit_quality", command, tmp_path, repo)
