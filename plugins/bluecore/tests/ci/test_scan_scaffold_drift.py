"""bluecore.ci.scan_scaffold_drift のテスト。"""
...
from bluecore.ci import scan_scaffold_drift as scanner
...
class TestKnownTags:
    ...
    def test_derived_from_definition_modules(self) -> None:
        ...
        from bluecore.lib.harness import COMMAND_TAGS, SCAFFOLD_TAGS
        from bluecore.mem.tag_stripping import STRIPPED_TAGS
...
class TestFindPairedUnknownTags:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "<bluecore-memory>x</bluecore-memory>",
            '<div class="a">x</div>',
        ],
    )
...
class TestMain:
    ...
    def test_module_entrypoint_runs(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(SystemExit) as excinfo:
            runpy.run_module("bluecore.ci.scan_scaffold_drift", run_name="__main__")
