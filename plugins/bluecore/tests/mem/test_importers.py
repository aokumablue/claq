...
from bluecore.mem.importers import (
...
class TestImportInstincts:
    ...
    def test_no_instincts_dir_returns_zero(self):
        from bluecore.mem.importers import import_instincts

        db = MagicMock()
        # 存在しないパスをモック
        with patch("bluecore.mem.importers.BLUECORE_DIR", Path("/nonexistent")):
    ...
    def test_parses_yaml_instinct_files(self, tmp_path):
        ...
        instincts_dir = tmp_path / ".bluecore" / "instincts" / "personal"
...
"""
        ...
        with patch("bluecore.mem.importers.BLUECORE_DIR", tmp_path / ".bluecore"):
            result = import_instincts(db, "test_user")
...
""",
    ...
    def test_project_filter_skips_global_dirs(self, tmp_path):
        ...
        db = MagicMock()
        global_dir = tmp_path / ".bluecore" / "instincts" / "personal"
        project_dir = tmp_path / ".bluecore" / "projects" / "proj-1" / "instincts" / "personal"
        other_project_dir = tmp_path / ".bluecore" / "projects" / "proj-2" / "instincts" / "personal"
        global_dir.mkdir(parents=True)
        project_dir.mkdir(parents=True)
        other_project_dir.mkdir(parents=True)
        (tmp_path / ".bluecore" / "projects" / "skip.txt").write_text("ignore", encoding="utf-8")
    ...
    def test_duplicate_project_dirs_are_skipped(self, tmp_path):
        ...
        first = bluecore_dir / "projects" / "proj-1" / "instincts" / "personal"
    ...
    def test_same_project_in_multiple_dirs_is_skipped(self, tmp_path):
        ...
        with patch("bluecore.mem.importers._project_dirs", lambda: [dir_a, dir_b]):
            result = import_instincts(db, "test_user")
...
class TestImportAdrs:
    ...
    def test_no_adr_dir_returns_zero(self, tmp_path):
        from bluecore.mem.importers import import_adrs
...
""",
        ...
        with patch("bluecore.mem.importers.Path.read_text", fake_read_text):
        ...
        with patch("bluecore.mem.importers.Path.read_text", side_effect=OSError("boom")):
...
class TestImportEventLogs:
    ...
    def test_parses_jsonl_files(self, tmp_path):
        ...
        bluecore_dir = tmp_path / ".bluecore"
        bluecore_dir.mkdir(parents=True)
        obs_file = bluecore_dir / "observations.jsonl"
        obs_file.write_text(
    ...
    def test_import_event_logs_honors_project_filter(self, tmp_path):
        ...
        db = MagicMock()
        bluecore_dir = tmp_path / ".bluecore"
        project_dir = bluecore_dir / "projects" / "proj-1"
        other_dir = bluecore_dir / "projects" / "proj-2"
        project_dir.mkdir(parents=True)
    ...
    def test_duplicate_project_dirs_are_skipped(self, tmp_path):
        ...
        db = MagicMock()
        bluecore_dir = tmp_path / ".bluecore"
        first = bluecore_dir / "projects" / "proj-1"
        first.mkdir(parents=True)
...
class TestImportAll:
    ...
    def test_calls_all_importers(self, tmp_path):
        from bluecore.mem.importers import import_all
    ...
    def test_import_all_aggregates_counts(self, tmp_path):
        ...
        db = MagicMock()
        with (
            patch("bluecore.mem.importers.import_instincts", return_value=1),
            patch("bluecore.mem.importers.import_adrs", return_value=2),
            patch("bluecore.mem.importers.import_event_logs", return_value=3),
        ):
