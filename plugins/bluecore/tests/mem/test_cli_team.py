...
from bluecore.mem import cli as cli_module
from bluecore.mem.settings import Settings, SyncSettings, TeamSettings
...
def test_team_context_skipped_when_sync_disabled(
    ...
    monkeypatch.setattr("bluecore.mem.team_context.build_team_context", _fake_build)
...
def test_team_context_silent_on_connection_failure(
    ...
    monkeypatch.setattr(
        "bluecore.mem.pg_database.PgDatabase",
...
def test_team_context_prints_additional_context(
    ...
    monkeypatch.setattr(cli_module, "get_git_user_name", lambda: "me")
    monkeypatch.setattr(
        "bluecore.mem.pg_database.PgDatabase", lambda url: pg_instance
    )
...
def test_team_session_init_requires_retrospective_prompt(
    ...
    monkeypatch.setattr(
        "bluecore.mem.team_context.build_team_context",
...
def test_team_context_and_session_init_failure_paths(
    ...
    fake_pg = ModuleType("bluecore.mem.pg_database")
    fake_team = ModuleType("bluecore.mem.team_context")
    monkeypatch.setitem(sys.modules, "bluecore.mem.pg_database", fake_pg)
    monkeypatch.setitem(sys.modules, "bluecore.mem.team_context", fake_team)
...
def test_team_context_build_raises_and_logs_warning(
    ...
    monkeypatch.setattr("bluecore.mem.team_context.build_team_context", _boom)
...
def test_team_session_init_import_failure(
    ...
    def _bad_import(name, *args, **kwargs):
        if name == "bluecore.mem.pg_database":
...
def test_search_and_inject_no_results(tmp_path, monkeypatch, capsys) -> None:
    """検索結果が空なら何も出力しない。"""
    from bluecore.mem.cli_session_handlers import _search_and_inject_context
    from tests.mem.conftest import FakeDB, make_settings
...
def test_team_session_init_empty_context(
    ...
    monkeypatch.setattr(cli_module, "get_git_user_name", lambda: "me")
    monkeypatch.setattr("bluecore.mem.pg_database.PgDatabase", lambda url: _FakePg(connected=True))
    monkeypatch.setattr("bluecore.mem.team_context.build_team_context", lambda *a, **kw: "")
    cli_module._handle_team_session_init(
