...
from bluecore.mem import bridge
...
def test_get_project_observations_path_creates_project_json(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert obs_path.parent == tmp_path / ".bluecore" / "projects" / "proj"
    project_json = obs_path.parent / "project.json"
    assert project_json.exists()
    assert '"project_id": "proj"' in project_json.read_text(encoding="utf-8")


def test_get_project_observations_path_uses_bluecore_projects_dir(
    ...
    obs_path = bridge._get_project_observations_path("proj", "project-name")
    assert obs_path == tmp_path / ".bluecore" / "projects" / "proj" / "observations.jsonl"
    assert obs_path.parent == tmp_path / ".bluecore" / "projects" / "proj"
...
def test_get_project_observations_path_existing_json(tmp_path, monkeypatch) -> None:
    ...
    import bluecore.mem.bridge as b
