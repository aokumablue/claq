...
def mock_git_user(monkeypatch):
    ...
    monkeypatch.setattr("bluecore.mem.sync.get_git_user_name", lambda: "test_user")
    return "test_user"
...
class TestShouldSync:
    ...
    def test_returns_true_just_after_retry_backoff_expires(self, mock_settings):
        ...
        from bluecore.mem.sync import _MIN_RETRY_INTERVAL
...
class TestSyncToPostgres:
    ...
    def test_pg_connection_failure_ignores_save_state_errors(self, mock_settings, monkeypatch, mock_git_user):
        ...
        pg_db = FakePgDb()
        monkeypatch.setattr("bluecore.mem.sync.Database", lambda path: sqlite_db)
        monkeypatch.setattr("bluecore.mem.sync.PgDatabase", lambda url: pg_db)
        monkeypatch.setattr(
            mock_settings,
            "save_sync_state",
...
class TestSyncLocking:
    ...
    def test_skips_when_another_sync_holds_lock(self, mock_settings, monkeypatch, mock_git_user):
        ...
        monkeypatch.setattr("bluecore.mem.sync.Database", FailingDatabase)
        monkeypatch.setattr("bluecore.mem.sync.PgDatabase", FailingPgDatabase)
...
class TestUpsertWithOrigin:
    ...
    def test_does_not_mutate_original(self):
        ...
        from bluecore.mem.sync import _upsert_with_origin
...
class TestSyncToPostgresDetailed:
    ...
    def test_full_sync_claims_rows_and_commits(self, mock_settings, monkeypatch, mock_git_user):
        ...
        monkeypatch.setattr("bluecore.mem.sync.Database", lambda path: sqlite_db)
        monkeypatch.setattr("bluecore.mem.sync.PgDatabase", lambda url: pg_db)
        monkeypatch.setattr("bluecore.mem.sync._sync_embeddings", lambda sqlite_db, pg_db, chunks: len(chunks))
        monkeypatch.setattr("bluecore.mem.sync.time.time", lambda: 10_000_000_000)
    ...
    def test_sync_check_delegates_when_needed(self, mock_settings, monkeypatch):
        monkeypatch.setattr("bluecore.mem.sync.should_sync", lambda settings: True)
        monkeypatch.setattr("bluecore.mem.sync.sync_to_postgres", lambda settings, dry_run=False: SyncResult(chunks=3))
...
class TestSyncHelpers:
    ...
    def test_dry_run_counts_embeddings_path(self, tmp_path, monkeypatch):
        ...
        from bluecore.mem.sync import _dry_run_counts

        db = Database(tmp_path / "test.db")
        monkeypatch.setattr("bluecore.mem.sync._count_pending_embeddings", lambda conn, ids: 42)
    ...
    def test_count_pending_rows_returns_zero_for_empty_table(self, tmp_path):
        ...
        from bluecore.mem.sync import _count_pending_rows
    ...
    def test_claim_pending_rows_raises_for_invalid_table(self, tmp_path):
        ...
        from bluecore.mem.sync import ClaimConfig, _claim_pending_rows, _row_to_chunk
    ...
    def test_count_pending_embeddings_handles_empty_and_operational_error(self):
        from bluecore.mem.sync import _count_pending_embeddings
...
class TestSyncLogVisibility:
    ...
    def test_sync_check_logs_info_on_skip_disabled(self, mock_settings, caplog):
        ...
        mock_settings.sync.enabled = False
        # get_logger("SYNC") は "bluecore.mem.SYNC" を返す
        with caplog.at_level(logging.INFO, logger="bluecore.mem.SYNC"):
            result = sync_check(mock_settings)
        assert result.success is True
    ...
    def test_sync_to_postgres_logs_error_on_connection_failure(self, mock_settings, monkeypatch, caplog):
        ...
        monkeypatch.setattr("bluecore.mem.sync.PgDatabase", FakePgDb)
        monkeypatch.setattr("bluecore.mem.sync.Database", FakeDatabase)

        with caplog.at_level(logging.ERROR, logger="bluecore.mem.SYNC"):
            result = sync_to_postgres(mock_settings)
        # 処理は継続し例外は出ない
        assert result.success is False
        assert result.error == "PostgreSQL への接続に失敗しました"
    ...
    def test_resolve_sync_lock_path_falls_back_when_path_invalid(self, monkeypatch):
        ...
        from bluecore.mem.sync import _resolve_sync_lock_path
        ...
        assert str(result).endswith(".bluecore/sync.lock")
    ...
    def test_reload_sync_state_falls_back_to_private_loader(self, monkeypatch):
        ...
        from bluecore.mem.sync import _reload_sync_state
    ...
    def test_mask_url(self, monkeypatch):
        ...
        from bluecore.mem.sync import _mask_url
