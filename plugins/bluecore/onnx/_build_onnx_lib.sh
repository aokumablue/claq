...
#   ~/.bluecore/.venv-modelbuild をビルド用 venv として使用する。
#
...
build_onnx_impl() {
  ...
  local build_venv="${HOME}/.bluecore/.venv-modelbuild"
  ...
  local log_dir="${HOME}/.bluecore/logs"
  ...
  # 再生成: pip-compile --allow-unsafe --generate-hashes --output-file=plugins/bluecore/onnx/requirements-build.txt plugins/bluecore/onnx/requirements-build.in
