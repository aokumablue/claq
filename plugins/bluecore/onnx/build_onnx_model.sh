...
#
# 使い方:
#   ./plugins/bluecore/onnx/build_onnx_model.sh                        # FP16 デフォルト（推奨）
#   ./plugins/bluecore/onnx/build_onnx_model.sh --quant fp32           # FP32（品質劣化ゼロ、約 1.2 GB）
#   ./plugins/bluecore/onnx/build_onnx_model.sh --quant int8           # INT8（動的量子化、約 300 MB）
#   ./plugins/bluecore/onnx/build_onnx_model.sh --quant fp16 --revision <SHA>
#
# 出力先: ~/.bluecore/models/（install.sh と共有）
# 使用する venv: ~/.bluecore/.venv-modelbuild（初回のみ自動作成）
#
# 量子化方式:
...
QUANT="fp16"
REVISION=""
OUT_DIR="${HOME}/.bluecore/models"

usage() {
  cat <<'EOF'
Usage: ./plugins/bluecore/onnx/build_onnx_model.sh [options]
...
Options:
  ...
  --out DIR                出力先ディレクトリ (default: ~/.bluecore/models)
  --help                   このヘルプを表示
EOF
}
