...
search: `mem search` — クエリ例 `architecture design ADR decision` / `pattern {pattern_name} implementation`。返るのは `- [kind] title (key)` の 1 行だけなので、本文が要る key だけ `mem show <key>` に渡す
