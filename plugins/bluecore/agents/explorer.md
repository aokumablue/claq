...
- 不要な同一ファイルの重複 Read は避ける（既読内容は記憶から参照）。ただしファイル変更の可能性や file:line 引用の正確性に確信が持てない場合は再 Read 可
- 返答は結論 summary を基本とする。生ログ・全文の丸ごと転記は避けるが、file:line 証拠の正確性担保に必要な数行の抜粋は許容
...
1. 指定されたスコープまたは調査予算に到達した
2. エントリポイント・コールチェーン・影響ファイルを、それぞれ file:line の証拠で確認した
3. 必要な情報が存在しない・アクセス不能・提供されていないと確認した

終了時は確認済みの範囲・証拠・未確認事項を出力する。必要な3点が証拠付きで確定した後、または不足情報を明示した後は追加調査をしない。周辺の興味深い実装・無関係な参照・推測を埋めるための探索には進まない。
