...
search: `mem search` — クエリ例 `performance baseline benchmark metrics` / `optimization improve bundle LCP`。返るのは `- [kind] title (key)` の 1 行だけなので、本文が要る key だけ `mem show <key>` に渡す
