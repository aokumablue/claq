...
1. **テスト改ざんガード（実行前・決定的・task_type 非依存）**: `git diff --staged` と `git diff` のテストファイル差分（`tests/` 配下・`test_*.py`・`*_test.py`）に (a) テスト関数・テストファイルの削除 (b) `@pytest.mark.skip`/`@pytest.mark.xfail` の新規付与 (c) assert 行のコメントアウト・`assert True`/`pass` への置換 のいずれかを検出したら、以降の再実行を行わず BLOCKER として報告
