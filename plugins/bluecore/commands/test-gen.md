...
- 参照: `bluecore_run bluecore.mem.cli search "..."`（`. "$HOME/.bluecore/env.sh"` 前提。クエリ例 `test test-gen coverage decision-table {対象ファイルパス}`）→ 本文が要る key だけ `bluecore_run bluecore.mem.cli show <key>`
