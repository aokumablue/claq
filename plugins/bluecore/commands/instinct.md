...
### import `<file-or-url>`
ローカルファイルまたは URL から取り込む。取込前に件数・出所を提示し、ローカルファイル・URL いずれもユーザー確認必須（差分提示 → 承認待機してから適用）。取込元による確認ゲートの非対称は設けない（汚染済みローカルファイルの無確認取込を防ぐ）。

### promote
昇格条件（2プロジェクト以上に出現・信頼度しきい値を満たす）の全候補を project → global へ自動昇格。

### prune
30日より古い未レビュー・未昇格の保留インスティンクトを削除。削除対象件数を先に提示し確認してから実行。
