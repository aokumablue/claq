...
**`HAPPY_TOOL` の出力は「その呼び出し位置で再現するか」で選ぶ。**
`smoke_check.py` は `structuredContent` を完全一致で比較する。基準は
「毎回新しい子プロセスで、同じ順番で呼んだときに同じ値が返るか」であって、
値が ID っぽいかどうかではない。

- 落ちる: UUID・現在時刻・乱数（起動ごとに変わる）
- 通る: プロセス内カウンタの採番 ID（**その呼び出しが毎回 1 回目なら** `1` で固定）

採番 ID でも、事前に別の呼び出しを挟む検査なら値がずれる。判断がつかないなら
数回続けて実行し、同じ値が返ることを確かめる。決定的にできない場合は、
その assert を必要な部分の比較へ書き換える。
...
**承認分岐が事前状態に依存するなら `setup=` を使う。** サーバはリクエストごとに
起動し直されるので、何も入っていないストアへの削除を承認しても「対象が無い」
エラーになり、MRTR の配線が正しいのか壊れているのか区別できない。
`setup=[("save_item", {...})]` のように、同一プロセスへ先に送る呼び出しを渡す。
