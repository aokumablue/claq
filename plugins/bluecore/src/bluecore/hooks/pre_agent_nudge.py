"""general-purpose サブエージェント起動時に専門エージェント対応表を提示します。

トリガー: PreToolUse (Task|Agent)
入力: subagent_type を含む tool_input JSON
出力: general-purpose 検知時のみ hookSpecificOutput に専門エージェント対応表を注入
終了: 0（いかなる場合もブロックしない）

general-purpose 以外の subagent_type、tool_input 欠落・非 dict 時は無出力で 0 を返す。
"""
...
def main() -> int:
    """general-purpose サブエージェント起動を検知して専門エージェント対応表を提示する。
