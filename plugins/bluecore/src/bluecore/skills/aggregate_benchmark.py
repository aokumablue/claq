#!/usr/bin/env python3
"""
個々の実行結果を集約し、benchmark サマリーを作成する。

run ディレクトリの grading.json を読み込み、次を出力する:
- 各指標の mean / stddev / min / max を持つ run_summary
- with_skill / without_skill の差分（delta）
...
def calculate_stats(values: list[float]) -> dict:
    """値のリストから mean / stddev / min / max を計算する。"""
    if not values:
...
def load_run_results(benchmark_dir: Path) -> dict:
    ...
    config 名（例: "with_skill" / "without_skill"、または "new_skill" / "old_skill"）を
    キーにした辞書を返し、それぞれに run 結果のリストを格納する。
    """
...
def aggregate_results(results: dict) -> dict:
    """
    run 結果を集約し、サマリー統計にまとめる。

    各 config の統計と delta を含む run_summary を返す。
    """
    run_summary = {}
...
def generate_benchmark(benchmark_dir: Path, skill_name: str = "", skill_path: str = "") -> dict:
    """run 結果から完全な benchmark.json を生成する。"""
    results = load_run_results(benchmark_dir)
    run_summary = aggregate_results(results)

    # benchmark.json 用の runs 配列を構築する
    runs = []
...
def generate_markdown(benchmark: dict) -> str:
    ...
    lines = [
        ...
        f"**日付**: {metadata['timestamp']}",
        f"**Eval**: {', '.join(map(str, metadata['evals_run']))}（各 config {metadata['runs_per_configuration']} 回実行）",
        "",
        "## サマリー",
        "",
...
def main():
    ...
    parser.add_argument("--skill-path", default="", help="ベンチマーク対象のスキルパス")
    parser.add_argument(
        "--output", "-o", type=Path, help="benchmark.json の出力先（既定: <benchmark_dir>/benchmark.json）"
    )
