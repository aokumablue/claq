...
@dataclass(frozen=True)
class DashboardDeps:
    """handle_dashboard の外部依存（DB接続・コールバック）。"""
...
def _build_item_ranking_vars(personal_ranking: list, personal_trend: list) -> dict:
    ...
    all_dates = sorted(personal_trend_by_date)
...
def _collect_personal_stats(open_db: Any, settings: Settings, days: int) -> tuple[list, list, list]:
    ...
    with open_db(settings) as db:
        conn = db.conn
        return (
            iq.item_usage_ranking(conn, days),
            iq.daily_trend(conn, days),
            iq.outcome_distribution(conn, days),
        )
