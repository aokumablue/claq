...
「祖先チェーン全体に writer が書き、`lstart`（プロセス起動時刻）で
個体識別する」設計へ作り直した。「決定」「結果」「リスク」節を全面差し替え、
改訂 1 の tier 2 は代替案として要約を残す。代替案 1〜3（初版由来、環境変数
判定を採らない理由・ランタイム集約 install を採らない理由）は変更なし。
...
1. `plugins/bluecore/src/bluecore/lib/env_pointer.py` の
   `write_env_pointer(plugin_root)` は、`os.getppid()` から**祖先チェーンを
   最大 4 段**まで辿り、各段の PID にもポインタを書く
   （`_resolve_ancestor_chain`）。1 回の `ps -eo pid=,ppid=,lstart=` で
   チェーンと各 PID の起動時刻（`lstart`）を同時に取得する（実測
   10〜16ms、hook のタイムアウト予算に対して無視できるコスト。全 hook
   起動でこの関数が走るため、1 回の呼び出しに抑えることを優先した）。
   `ps` の実行に失敗した場合はその回は何も書かない
   （ADR-0001 の fail-open。次回 hook 起動で自己修復する）。
2. 各ポインタは `root\nlstart\n` の 2 行。`.`/`source` はせず、
   ...
   持つのは同一プロセスの生存中に限られる）。
3. 祖先 PID に**既に別の root** が記録されていたら、`_write_or_poison_pointer`
   は上書きせず内容を空にする（**poison**）。同じ terminal / login shell
   から異なる複数の host が起動され、浅い祖先 PID を共有してしまう
   ケースの対処: 後勝ち上書きだと片方が誤って相手の root を掴む経路が
   残るが、poison にすれば resolver はその祖先をスキップして次へ進む
   ため、「どちらか一方が誤って相手を掴む」という事象自体が起こらない。
   自分により近い祖先（通常は host バイナリ自体の PID）は共有されにくい
   ため、共有される深い祖先が poison になっても各 host 自身の解決能力は
   損なわれない。一度 poison になったポインタは、その PID が生存し
   続ける限り誰にも上書きされない（poison の恒久性）。
4. `plugins/bluecore/runtime/env-template.sh` は自分の `$PPID` から
   祖先を最大 4 段 `ps -o ppid=` で辿り、各段で `_bluecore_resolve_ancestor_pointer`
   を呼ぶ: pointer が 2 行とも揃っていて、かつ `LC_ALL=C ps -o lstart= -p <pid>`
   で得た**現在の** lstart が記録値と一致する場合のみ採用する。空
   （poison）・欠落・不一致（PID 再利用）はスキップして次の祖先へ進む。
   **祖先チェーンで解決できなければ、推測せず常に `exit 127`**
...
### 代替案 4（改訂 1 で採用、本改訂で撤回）: 祖先で解決できない場合、
...
- `latest` の無条件採用（R-05）を全廃し、**祖先チェーンで `lstart` が
  ...
  無くなった。writer 側が祖先チェーン全体に（root 値の重複はあっても）
  同じ root を書き続けるため、祖先が浅い段で一致する限り、writer 側の
  `os.getppid()` が短命な中間 shell を指すケース（reader/writer の
  非対称性）でも解決できる。writer 側は既存レビュー（改訂 1 時点の
  advisor 指摘）で発覚していた「候補ちょうど 1 本」規則の脆弱性を
  そもそも持たない（file count に依存する判定自体が無いため）。
- 祖先で解決できない場合に、同じ terminal / login shell を共有する
  別ホストの root と衝突したときは poison（空ファイル）にすることで、
  「どちらか一方が誤って相手を掴む」事象を「双方とも使わない」に
  変えた。poison は該当 PID が生存する限り恒久的なので、後から
  どちらかが再度上書きして復活させることもない。
...
- 共有される祖先 PID（terminal / login shell 等）に poison が溜まる
  ことがあるが、GC がいずれ回収する（生存 PID の場合は TTL、PID が
  死ねば猶予後に削除）ため無制限には残らない。
...
- `ps -o lstart=` の出力はロケール依存であることを実装中に発見した
  ...
  確認済み。
- 祖先チェーンの深さは 4 段に固定している。host が 4 段を超える
  中間 shell を挟んで hook を起動する構成では、writer が書いた祖先
  ポインタに reader の祖先 walk が届かず 127 になりうる。実測では
  Claude Code の構成（launcher の親はホストが起動する使い捨てラッパー
  shell、その親が host バイナリ、その親がログインシェル）で 4 段は
  十分だった（`docs/reports/PLUGIN_ROOT_RESOLVER_2026-08-20_V0.9.36_REVERIFICATION.md`
  の検証環境と同様の構成を想定）が、他ホストでの実測は未確認。
