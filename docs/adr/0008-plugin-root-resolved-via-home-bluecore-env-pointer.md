...
1. `plugins/bluecore/runtime/env-template.sh`（実 shell file）が次の解決
   ...
   2. 祖先チェーンで解決できず、かつ `roots/` のうち**直近 5 分以内に
      書かれた有効なポインタが全て同じ root 値に一致する**場合はそれを
      採用する（「候補がちょうど 1 本」ではなく「候補が合意している」
      ことを条件にした。単独ホストが launcher 起動のたびに異なる短命
      PID を記録し続ける場合でも、root 値さえ一致していれば解決できる。
      異なるホスト／異なる install はほぼ確実に異なる path を記録する
      ため、不一致は「どちらのホストか判別できない」信頼できる signal
      になる。5 分の鮮度フィルタは `find <pointer> -maxdepth 0 -mmin -5`
      で判定する — `stat` はフォーマット指定が BSD/GNU で非互換
      〔ADR-0009〕だが `-mmin`/`-maxdepth` は両方で使える。このフィルタが
      無いと、使われなくなった別ホストのポインタが GC される最大 1 時間、
      単独稼働中のホストまで巻き込んで 127 にしてしまう）。
   3. いずれも解決できなければ明示メッセージを stderr に出し `exit 127`
...
- `latest` の無条件採用（R-05）を廃し、「直近 5 分の候補が対立し祖先も
  一致しないときだけ 127」に変えたことで、複数ホスト同時稼働時に別ホスト
  の install を誤って source する経路を塞いだ。単独ホスト運用（最も
  一般的な形態）は「直近 5 分の有効な候補が全て同じ root 値」規則で
  常に解決する。この規則は writer 側の `os.getppid()` が短命な中間
  shell を指し、hook 起動ごとに異なる PID の pointer が乱立するケース
  でも成立する（全て同じ root 値を書くため）——「候補がちょうど 1 本」
  という file count ベースの規則ではこの場合ほぼ常に「候補 2 本以上」に
  なり 127 になっていたはずで、実装レビュー（advisor）で file count 版の
  欠陥として指摘され、root 値一致版へ修正した。5 分の鮮度フィルタも
  同じレビューで追加した: フィルタが無いと、使われなくなった別ホストの
  ポインタが GC される最大 1 時間（GC の throttle 周期）、単独稼働中の
  ホストまで 127 に巻き込んでいた。
...
- 直近 5 分以内に別ホストが（今は稼働していなくても）hook を起動して
  いた場合、単独ホストでも祖先チェーンが外れると 127 になりうる
  （鮮度ウィンドウ内は「対立」として扱われるため）。5 分は GC の
  1 時間 throttle より大幅に狭いが、ゼロではない。ユーザーへの影響は
  「直前に別ホストを触っていた場合に限り、最大 5 分だけ再解決を待たされる」
  程度に抑えている。
