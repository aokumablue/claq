...
GC では見えないため、`~/.ple4` 直下で同じ age-gate を適用する。

**改訂 5（本改訂）**: 2026-08-26 の実機監査 F-11 を受け、`ple4-helpers.sh`
...
`_PLE4_SOURCED_ROOT` で helper へ明示的に引き渡す形にした。POSIX sh は
...
`ple4_run`/`ple4_mem_learn`（`runtime/ple4-helpers.sh`
の shell 関数）を参照する agents/commands/skills の md のうち、bootstrap
（`. runtime/ple4-helpers.sh` を実行するコード）を持たない 16 surface
が、実行すると `ple4_run: command not found`（exit 127）になることを
...
**plugin root の解決を `~/.ple4/env.sh`（launcher が全 hook 起動時に
...
1. `plugins/ple4/src/ple4/lib/env_pointer.py` の
...
3. 深さを **2 段に絞る**のは、共有されうる祖先へ書き込むこと自体を
   ...
   事前回避へ改めた。下記代替案参照）。
4. `plugins/ple4/runtime/env-template.sh` は自分の `$PPID` とその親
   （**最大 2 段**）を `ps -o ppid=` で辿り、各段で
   `_ple4_resolve_ancestor_pointer` を呼ぶ: pointer が 2 行とも
...
9. agents/commands/skills の md は
   `. "$HOME/.ple4/env.sh" || exit 127` の 1 行だけを書く。ベンダ固有
...
### 代替案 3: ランタイム実装（Python/Shell）を `~/.ple4` へ集約インストールし、
...
- 短所:
  ...
  - プラグインのバージョンとランタイムのバージョンが分離し、
    「md（プラグイン側）はバージョン更新されたがランタイム（`~/.ple4`
...
- M-02 の 16 surface（bootstrap 欠如）と 11 箇所の重複ループが、
  1 行の共通 bootstrap （`. "$HOME/.ple4/env.sh" || exit 127`）に
  統一された。
...
- 書き込みが原子的になり（`os.replace()`）、並行 hook 起動時の partial
  ...
  `~/.ple4` 直下に作られるため、同じ age-gate を prefix
...
- 祖先チェーンの深さは 2 段（`os.getppid()` とその親）に固定している。
  ...
  （代替案 5 参照）。
- Copilot CLI での実機動作（`~/.ple4/roots/` が実際に生成され、
  ...
  チェックリストで別途実施する。実施時は `~/.ple4/roots/` の実
