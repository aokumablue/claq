...
`~/dev/bluecore/plugins/bluecore` と、実際にCopilot CLIへ導入されていた
`~/.copilot/installed-plugins/bluecore/bluecore` の配布対象86ファイルはSHA-256で一致した。したがって、インストール済みv0.9.41で採取した実機証跡を現HEADへ適用できる。
...
- `claude-bluecore-workaround-details.log`: agent登録workaroundの対照実験
- `adr-critic-report.md`: ADR全件の専門agent批評

### 4.3 characterization testの再実行

```bash
AUDIT=~/.copilot/session-state/13c178b0-42e2-48c3-b39e-921b49fa562f/files/bluecore-audit
PYTHONPATH=~/dev/bluecore/plugins/bluecore/src \
...
| `plan` | FAIL | 計画本文は生成したが、定義上必須の`bluecore:planner`を起動しなかった |
...
| 対象 | 実測 |
|---|---|
| launcher同期 | `bluecore.mem.cli --help` exit 0 |
...
- `plugins/bluecore/.claude-plugin/plugin.json:19-29`
- Claude Code 2.1.220

**期待**

manifestに列挙された9 agentが`bluecore:<name>`として利用可能になる。
...
claude plugin validate --strict \
  ~/dev/bluecore/plugins/bluecore

claude --plugin-dir \
  ~/dev/bluecore/plugins/bluecore \
  plugin details bluecore@inline

claude --debug-file \
  "$AUDIT/claude-plugin-details-debug.log" \
  --plugin-dir ~/dev/bluecore/plugins/bluecore \
  plugin details bluecore@inline
```
...
Claude Code 2.1.220のloaderは、validatorと公式schemaが許可するagentファイル配列を受理する一方、runtimeでは各パスをdirectoryとして`scandir`している。過去版との比較はしていないためregressionとは断定せず、bluecoreのmanifestとClaude Code 2.1.220 runtimeの非互換と評価する。
...
2. release gateへ次を追加する。
   ...
   ```bash
   claude plugin validate --strict plugins/bluecore
...
- 欠落している`plugins/bluecore/tests/`
- ADR-0006

**再現**

```bash
cd ~/dev/bluecore/plugins/bluecore
...
- Claude runtimeでAgents 0になるmanifestも、bluecore validatorと`claude plugin validate --strict`の双方がPASS
...
Python 3.11相当では`bluecoreProtectionDisabled=true`をstderrへ出すが、exit 0で全hookを許可する。
...
- `runtime/bluecore-helpers.sh:10-18`
- `lib/env_pointer.py`
- ADR-0008、0009
...
1. 同一UIDで親PIDと`ps -o lstart=`が一致する
   `~/.bluecore/roots/<pid>`を作成する。
2. pointerのrootを監査用fake pluginへ向ける。
3. `env-template.sh`をsourceする。
4. fake `runtime/bluecore-helpers.sh`がsourceされる。

加えて、正規pointerからhelperをsourceした後も
`bluecore_plugin_root()`はambient `CLAUDE_PLUGIN_ROOT`を優先する。
...
- `~/.grok/installed-plugins/bluecore-*`の最初の候補へfixture moduleを置くと、そのmodule初期化codeがimport時に実行された。
- Grok未導入fixtureでは
  `no installed bluecore-* found...`を表示してexit 0。
...
全runは`bluecore:comparator`、`gpt-5.6-sol`、`reasoning_effort=medium`、sync modeで実行した。injection 5回は入力A/B、task、expectationsを固定した。dispatch文言は初回、retry、run 3〜5で等価な3表現を使い、output pathとagent nameもrunごとに変えている。
...
`~/.bluecore`は通常0700へ補正されるため、symlinkを作れる主体は原則として同一UIDであり、対象DBを直接変更できる。したがって権限昇格としてはLOWだが、誤設定や復元事故で別DBを破壊する運用上の完全性問題はMEDIUMである。
...
- `runtime/env-template.sh`
- `runtime/bluecore-helpers.sh:10`

**再現**

```bash
dash -c '. plugins/bluecore/runtime/bluecore-helpers.sh'
```
...
- `runtime/bluecore-helpers.sh:110-117`
...
- `README.md:393,408`
- ADR-0007
- `runtime/bluecore-helpers.sh`
- `mem/cli.py`

**実測**

READMEは高確信度時に
`bluecore_mem_learn --status active`
...
```bash
python -m build --wheel plugins/bluecore
```

buildは成功するが、wheelは`src/bluecore`配下のPython packageだけを含み、次を含まない。
...
READMEの公式配布経路はClaude marketplaceであり、pip/wheelとは書かれていないため、現時点では製品不具合と断定しない。ただし`[project]`とbuild backendが存在するため、wheelを公開・配布した場合は「bluecore」という名前の非機能artifactになる。
...
- `ruff check plugins/bluecore/src`: PASS
