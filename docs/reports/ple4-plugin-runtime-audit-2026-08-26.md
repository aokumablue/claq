# ple4 v0.9.41 全機能実機監査報告

> **査読と対応の記録（2026-08-27 追記）**
>
> 本レポートの全 30 件を `ple4-dev` の HEAD で再現検証し、対応した。判定は
...
> `--root plugins/ple4` 指定で **45/58**（本レポートは 40/58）。対応後は 58/58。
>
> **パスの訂正**: 本文中の `plugins/ple4/hooks/*.py` / `launcher.py` /
> `lib/` / `ci/` は、本リポジトリでは `plugins/ple4/src/ple4/` 配下にある。

## 1. 結論

`ple4` v0.9.41 の公開サーフェスを、GitHub Copilot CLI、Claude Code のローカルプラグインローダー、隔離fixture、launcher直呼び出しで検証した。
...
`~/dev/ple4/plugins/ple4` と、実際にCopilot CLIへ導入されていた
`~/.copilot/installed-plugins/ple4/ple4` の配布対象86ファイルはSHA-256で一致した。したがって、インストール済みv0.9.41で採取した実機証跡を現HEADへ適用できる。
...
- `claude-ple4-workaround-details.log`: agent登録workaroundの対照実験
- `adr-critic-report.md`: ADR全件の専門agent批評

### 4.3 characterization testの再実行

```bash
AUDIT=~/.copilot/session-state/13c178b0-42e2-48c3-b39e-921b49fa562f/files/ple4-audit
PYTHONPATH=~/dev/ple4/plugins/ple4/src \
...
| `plan` | FAIL | 計画本文は生成したが、定義上必須の`ple4:planner`を起動しなかった |
...
| 対象 | 実測 |
|---|---|
| launcher同期 | `ple4.mem.cli --help` exit 0 |
...
- `plugins/ple4/.claude-plugin/plugin.json:19-29`
- Claude Code 2.1.220

**期待**

manifestに列挙された9 agentが`ple4:<name>`として利用可能になる。
...
claude plugin validate --strict \
  ~/dev/ple4/plugins/ple4

claude --plugin-dir \
  ~/dev/ple4/plugins/ple4 \
  plugin details ple4@inline

claude --debug-file \
  "$AUDIT/claude-plugin-details-debug.log" \
  --plugin-dir ~/dev/ple4/plugins/ple4 \
  plugin details ple4@inline
```
...
Claude Code 2.1.220のloaderは、validatorと公式schemaが許可するagentファイル配列を受理する一方、runtimeでは各パスをdirectoryとして`scandir`している。過去版との比較はしていないためregressionとは断定せず、ple4のmanifestとClaude Code 2.1.220 runtimeの非互換と評価する。
...
2. release gateへ次を追加する。
   ...
   ```bash
   claude plugin validate --strict plugins/ple4
...
- 欠落している`plugins/ple4/tests/`
- ADR-0006

**再現**

```bash
cd ~/dev/ple4/plugins/ple4
...
- Claude runtimeでAgents 0になるmanifestも、ple4 validatorと`claude plugin validate --strict`の双方がPASS
...
Python 3.11相当では`ple4ProtectionDisabled=true`をstderrへ出すが、exit 0で全hookを許可する。
...
1. 同一UIDで親PIDと`ps -o lstart=`が一致する
   `~/.ple4/roots/<pid>`を作成する。
2. pointerのrootを監査用fake pluginへ向ける。
3. `env-template.sh`をsourceする。
4. fake `runtime/ple4-helpers.sh`がsourceされる。

加えて、正規pointerからhelperをsourceした後も
`ple4_plugin_root()`はambient `CLAUDE_PLUGIN_ROOT`を優先する。
...
- `~/.grok/installed-plugins/ple4-*`の最初の候補へfixture moduleを置くと、そのmodule初期化codeがimport時に実行された。
- Grok未導入fixtureでは
  `no installed ple4-* found...`を表示してexit 0。
...
全runは`ple4:comparator`、`gpt-5.6-sol`、`reasoning_effort=medium`、sync modeで実行した。injection 5回は入力A/B、task、expectationsを固定した。dispatch文言は初回、retry、run 3〜5で等価な3表現を使い、output pathとagent nameもrunごとに変えている。
...
`~/.ple4`は通常0700へ補正されるため、symlinkを作れる主体は原則として同一UIDであり、対象DBを直接変更できる。したがって権限昇格としてはLOWだが、誤設定や復元事故で別DBを破壊する運用上の完全性問題はMEDIUMである。
...
- `runtime/env-template.sh`
- `runtime/ple4-helpers.sh:10`

**再現**

```bash
dash -c '. plugins/ple4/runtime/ple4-helpers.sh'
```
...
- `runtime/ple4-helpers.sh:110-117`
...
- `README.md:393,408`
- ADR-0007
- `runtime/ple4-helpers.sh`
- `mem/cli.py`

**実測**

READMEは高確信度時に
`ple4_mem_learn --status active`
...
```bash
python -m build --wheel plugins/ple4
```

buildは成功するが、wheelは`src/ple4`配下のPython packageだけを含み、次を含まない。
...
READMEの公式配布経路はClaude marketplaceであり、pip/wheelとは書かれていないため、現時点では製品不具合と断定しない。ただし`[project]`とbuild backendが存在するため、wheelを公開・配布した場合は「ple4」という名前の非機能artifactになる。
...
- `ruff check plugins/ple4/src`: PASS
...
> repo モードで **45/58**（`--root plugins/ple4` が必要。リポジトリルートを
